...
expected=$(python3 -c 'import json; print(json.load(open("bench/configs/stage4-actions-ab.json"))["agents"][0]["kwargs"]["snapshot_sha256"])')
test "$(sha256sum "$snapshot/manifest.json" | cut -d' ' -f1)" = "$expected"
# Same empty task image pinned by bench/adapters/deadline_gate.py; the capture image already contains bench.
...
docker run -d --name "$fixture" --label pi-odoo-harness-lab=stage4 \
    ...
    -v "$(pwd)/bench/adapters/snapshot_entrypoint.sh:/lab-snapshot-entrypoint.sh:ro" \
    -v "$(pwd)/bench/adapters/snapshot.py:/lab-snapshot.py:ro" \
...
docker exec -e PYTHONPATH="$remote:/tmp/pi-odoo-mcp-source" \
    ...
    "$fixture" /bin/bash -c \
    'export ODOO_API_KEY="$(cat /etc/odoo/api_key)" ODOO_PASSWORD="$(cat /etc/odoo/api_key)"; exec "$1" -m bench.adapters.action_gate' \
