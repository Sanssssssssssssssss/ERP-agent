...
async def _install_task_mcp(agent: Any, environment: BaseEnvironment) -> None:
    ...
    await environment.upload_dir(
        root / "bench/reference/mcp/src/odoo_mcp", "/tmp/pi-odoo-mcp-source/odoo_mcp"
    )
    packages = shlex.join(
        (
            "odoo-mcp==1.3.2",
...
async def _install_task_runtime(
    ...
    if not native_only:
        await agent.exec_as_root(environment, command="mkdir -p /tmp/erp-harness-reference")
        for name in ("runner.py", "pi_mcp.py"):
            await environment.upload_file(root / "bench/reference" / name, f"/tmp/erp-harness-reference/{name}")
    wheel_hash = hashlib.sha256(wheels[0].read_bytes()).hexdigest()
    await agent.exec_as_root(environment, command=(
        f"echo {shlex.quote(wheel_hash)} > /logs/agent/backend-wheel.sha256"
    ))
    ...
    async def _run(self, instruction: str, environment: BaseEnvironment,
        ...
        command = (
            ...
            + ('/tmp/pi-odoo-env/bin/python -m erp_harness.app.runner '
               if native_only else '/tmp/pi-odoo-env/bin/python /tmp/erp-harness-reference/runner.py ')
            +
