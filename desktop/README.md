...
运行详情支持调用树、时间轴、订单/工具搜索和首个记录异常定位。点击节点才读取完整内容；
历史选择保持原位，点击“跟随最新”才跟随运行。模型请求可展开实际消息、工具定义、相邻请求变化；
业务动作可查看审批依据、规范化载荷与回读回执。字符统计不等于 token；HTTP 200 不证明模型流或业务完成。
旧日志没有请求关联或时间时显示未知。新请求的元信息和公开输出保存在当前 profile 的
`data/runs/<run_id>/requests/*.meta.json`、`*.output.json`，原始请求仍为 `*.request.json`。
查看这些详情不调用模型，也不执行 ERP 操作。界面过滤隐藏推理与凭据字段；原始日志仍按业务数据保管。
