...
export function businessTypeMeta(type?: string) { return ({ sale_invoice: { title: '销售与开票', short: '销售发票' }, purchase: { title: '采购', short: '采购流程' }, sale_purchase_invoice: { title: '销售 → 采购 → 开票', short: '业务链' } } as Record<string, { title: string; short: string }>)[type || ''] || { title: '业务工作区', short: '业务' } }

export function completionTargetLabel(target?: string, type?: string) { const effective = target || (type === 'purchase' ? 'confirmed' : 'posted'); return ({ read_only: '只读浏览', draft: '保留草稿', confirmed: '完成确认', posted: '发票已过账' } as Record<string, string>)[effective] || '完成目标未知' }
...
export function stageLabel(stage?: string) { return ({ material: '材料', read: '读取', quote: '报价', sales: '销售', confirm: '确认', purchase: '采购', invoice: '开票', verify: '核验' } as Record<string, string>)[stage || ''] || '未知阶段' }
...
export function outcomeScopeLabel(scope?: string) { return ({
...
export function modelLabel(model: string) { return model === 'sale.order' ? '销售订单' : model === 'purchase.order' ? '采购订单' : model === 'account.move' ? '客户发票' : model === 'account.move.send.wizard' ? '发票文件向导' : model === 'res.partner' ? '往来单位' : model }
