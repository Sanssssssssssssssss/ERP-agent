...
from pi_agent.session.jsonl import entries_from_json_lines, entry_to_json_line
...
def _lock_file(file: BinaryIO, *, exclusive: bool) -> None:
    ...
    if os.name == "nt":
        ...
        del exclusive
        msvcrt_api.locking(file.fileno(), msvcrt_api.LK_LOCK, 1)
        return
    import fcntl

    fcntl_api: Any = fcntl
    mode = fcntl_api.LOCK_EX if exclusive else fcntl_api.LOCK_SH
    fcntl_api.flock(file.fileno(), mode)
