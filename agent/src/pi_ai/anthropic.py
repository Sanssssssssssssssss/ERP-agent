...
from pi_ai.tool_call_ids import normalize_portable_tool_call_id, portable_tool_call_id
...
def _apply_message_cache_breakpoints(
    ...
    back from a breakpoint when searching for a reusable prefix, and one pi-agent turn
...
def _previous_request_boundary(messages: list[dict[str, JSONValue]]) -> int | None:
    ...
    Pi's transcript is append-only and every request stops immediately before the
...
def _usage_from_message_start(raw: object) -> Usage:
    ...
    (None) because Pi has no per-model pricing table.
    """
