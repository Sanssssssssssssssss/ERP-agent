...
from pi_ai.model_limits import RuntimeModelLimits
from pi_ai.openai_cache import openai_prompt_cache_key
...
from pi_ai.tool_call_ids import pi_short_hash, responses_tool_call_id
    ...
    *,
    ...
    messages = transform_messages(
        ...
        normalize_tool_call_id=lambda value, source: responses_tool_call_id(
            ...
            target_api="openai-codex-responses",
        ),
    )
...
def _messages_to_responses_input(
    messages: list[AgentMessage], *, model: str, supports_images: bool = False
) -> list[JSONValue]:
    ...
    for message_index, message in enumerate(messages):
        if isinstance(message, UserMessage):
            ...
            same_provider_and_api = (
                message.provider == "openai-codex" and message.api == "openai-codex-responses"
            )
            ...
            for block in message.content:
                if isinstance(block, ThinkingContent) and block.thinking_signature:
                    ...
                    fallback_id = (
                        f"msg_pi_{message_index}"
                        if text_block_index == 0
                        else f"msg_pi_{message_index}_{text_block_index}"
                    )
                    text_block_index += 1
                    message_id = signature_id or fallback_id
                    if len(message_id) > 64:
                        message_id = f"msg_{pi_short_hash(message_id)}"
                    message_item: dict[str, JSONValue] = {
                        "type": "message",
                        "role": "assistant",
                        "content": [
                            {
...
def _usage_from_response(event: Mapping[str, Any]) -> Usage | None:
    ...
    input. Cost is left unset (None) because Pi has no per-model pricing table.
    """
    response = event.get("response")
    if not isinstance(response, Mapping):
        return None
    ...
    *,
    ...
    headers = {
        **dict(configured_headers or {}),
        ...
        "User-Agent": f"pi-agent ({system()} {release()}; {machine()})",
