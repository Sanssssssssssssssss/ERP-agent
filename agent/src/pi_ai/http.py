"""HTTP client helpers shared by Pi network integrations."""
...
def normalize_proxy_url(proxy_url: str) -> str:
    ...
    request. Treat the generic form as SOCKS5 so Pi can honor these proxy
...
def create_async_client(**kwargs: Any) -> httpx.AsyncClient:
    """Create an ``httpx.AsyncClient`` with Pi's proxy normalization applied."""
...
def get_json(url: str, *, timeout: float, follow_redirects: bool = False) -> dict[str, object]:
    """Fetch a JSON object with Pi's proxy normalization applied."""
