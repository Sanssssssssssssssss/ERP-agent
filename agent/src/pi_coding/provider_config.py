"""Durable provider configuration for Pi coding sessions."""
...
from pi_ai.openai_codex import DEFAULT_OPENAI_CODEX_BASE_URL
from pi_coding.catalog_loader import effective_catalog, save_user_catalog_entries
from pi_coding.credentials import FileCredentialStore, credentials_path
from pi_coding.oauth_registry import get_oauth_provider
...
class ProviderConfigError(ValueError):
    """Raised when Pi provider configuration is invalid."""
...
class ProviderSettings:
    """Pi provider settings loaded from Pi home."""
...
class ProviderSelection:
    """Resolved provider/model selection for a Pi run."""
...
def builtin_provider_configs() -> tuple[ProviderConfig, ...]:
    """Return Pi's built-in provider configs."""
    return tuple(
...
def default_openai_provider_config() -> OpenAICompatibleProviderConfig:
    """Return Pi's default OpenAI-compatible provider entry."""
...
def provider_settings_path(paths: PiPaths | None = None) -> Path:
    """Return the durable provider settings path."""
    return (paths or PiPaths()).home / "providers.json"
...
def load_provider_settings(paths: PiPaths | None = None) -> ProviderSettings:
    ...
    resolved_paths = paths or PiPaths()
...
def save_provider_settings(settings: ProviderSettings, paths: PiPaths | None = None) -> Path:
...
def _save_migrated_provider_settings(settings: ProviderSettings, *, paths: PiPaths) -> None:
...
def _effective_provider_configs(paths: PiPaths | None = None) -> tuple[ProviderConfig, ...]:
    ...
    *,
    ...
    for name, preference_data in value.items():
        ...
        if provider_name not in catalog_configs:
            ...
            # preference behind. Ignore it so one stale entry cannot prevent Pi
            # from starting or running `pi-agent setup` to register it again.
            continue
        providers.append(
...
def _apply_provider_preference(
    ...
    # Provider preferences are user-level state shared across Pi versions.
    ...
    *,
    ...
    if provider.thinking_levels is None:
        ...
        if isinstance(provider, OpenAICodexProviderConfig):
            return (
                "OpenAI Codex subscription can stream reasoning output, but Pi does "
...
def provider_has_usable_credentials(
    ...
    *,
    ...
    """Return whether Pi can attempt calls for this provider without prompting setup."""
...
def _reasoning_effort_from_provider(
    ...
    *,
    ...
    if provider.name == "huggingface" and normalized == "minimal":
        ...
        # for reasoning_effort, but rejects Pi/Pi's "minimal" label.
        return "low"
