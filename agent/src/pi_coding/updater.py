"""Upgrade Pi with the package manager that owns its environment."""
...
from pi_coding.update_check import PYPI_PACKAGE_NAME, fetch_latest_pypi_version
...
class UpdateResult:
    """Result of trying to upgrade Pi."""
...
def update_pi_agent(
    *,
    ...
    """Upgrade Pi with the installer that owns the active environment.
    ...
    if direct_url:
        return _failure(
            "Pi was installed from a local or direct URL, so it cannot be safely "
    ...
    if method is None:
        if (prefix / "conda-meta").is_dir():
            return _failure(
                "Pi is installed in a Conda/Pixi-managed environment. "
                "Update it with the manager that created that environment."
            )
        installed_by = installer or "unknown"
        return _failure(
            "Could not identify a supported installer for this Pi environment "
    ...
    if method == "uv-tool":
        try:
            ...
            return _failure(f"Could not determine the latest Pi version from PyPI: {exc}")
        if latest_version is None:
            return _failure("Could not determine the latest Pi version from PyPI.")
...
try {
    ...
    $WaitMessage = "Waiting for Pi process $ParentProcessId to exit..."
    ...
    try {
        ...
        if ($_.FullyQualifiedErrorId -like "NoProcessFoundForGivenId*") {
            Write-Log "Pi process $ParentProcessId has already exited."
        } else {
            $Detail = $_.Exception.Message
            throw "Could not inspect Pi process $ParentProcessId. Update aborted: $Detail"
        }
    }
    if ($null -ne $ParentProcess) {
        try {
            ...
            $Detail = $_.Exception.Message
            throw "Could not wait for Pi process $ParentProcessId to exit. Update aborted: $Detail"
        }
    }
...
def _handoff_windows_update(
    ...
    *,
    ...
    try:
        if owns_update_dir:
            update_dir = Path(tempfile.mkdtemp(prefix="pi-agent-update-"))
        else:
    ...
    return UpdateResult(
        command=command,
        stdout=(
            "Pi update is scheduled and will start after this process exits. "
