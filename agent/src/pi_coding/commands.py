"""Slash command registry for Pi coding sessions."""
...
from pi_coding.provider_catalog import BUILTIN_PROVIDER_CATALOG, builtin_provider_entry
...
from pi_coding.thinking import normalize_thinking_level
...
def create_default_command_registry() -> CommandRegistry:
    """Create Pi's built-in slash command registry."""
    registry = CommandRegistry()
    registry.register(
        SlashCommand(
            name="quit",
...
def _theme_command(context: CommandContext) -> CommandResult:
    ...
    # Imported lazily so importing this module never pulls in `pi_coding.tui`
    # (whose package __init__ imports Textual) until /theme actually executes.
    from pi_coding.tui.themes import available_tui_theme_names
