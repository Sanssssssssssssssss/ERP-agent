"""Best-effort PyPI update checks for the Pi CLI."""
...
from pi_ai.http import get_json
from pi_coding.paths import PiPaths

PYPI_PACKAGE_NAME = "pi-agent-python"
...
UPDATE_CHECK_ENV_DISABLE = "PI_AGENT_NO_UPDATE_CHECK"
...
class UpdateNotice:
    ...
    def message(self) -> str:
        ...
        return (
            f"Pi {self.latest_version} is available (installed: {self.current_version}). "
            "Run `pi-agent update` to upgrade."
        )
...
class ReleaseNotesEntry:
    """Structured release notes for one Pi version."""
...
class ReleaseNotesNotice:
    """Release notes shown once after the installed Pi version changes."""
    ...
    def message(self) -> str:
        ...
        return f"Pi updated to {self.current_version}\n\n{body}"
...
def startup_update_notice(
    ...
    *,
    ...
    """Return an update notice when PyPI has a newer stable Pi release.
...
def startup_release_notes_notice(
    ...
    *,
    ...
    """Return release notes once after Pi starts with a newer installed version.
...
def fetch_latest_pypi_version(*, fetcher: Fetcher | None = None) -> str | None:
    """Fetch the latest stable Pi version from PyPI."""
...
def default_update_check_cache_path(paths: PiPaths | None = None) -> Path:
    """Return the on-disk cache path for startup update checks."""
    return (paths or PiPaths()).home / "cache" / "update-check.json"


def default_release_notes_state_path(paths: PiPaths | None = None) -> Path:
    """Return the on-disk state path for one-time release notes."""
    return (paths or PiPaths()).home / "cache" / RELEASE_NOTES_STATE_FILENAME
