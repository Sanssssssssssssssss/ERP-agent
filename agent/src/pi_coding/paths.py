"""Canonical filesystem paths for Pi user and project data."""
...
class PiPaths:
    """Resolved Pi filesystem locations.

    Pi keeps durable application data under the user's home directory while also
    loading project-local resources from the active working directory.
    """

    home: Path = field(default_factory=lambda: Path.home() / ".pi-agent")
    ...
    def logs_dir(self) -> Path:
        """Return Pi's user-level diagnostic log directory."""
    ...
    def user_skills_dir(self) -> Path:
        """Return Pi's user-level skills directory."""
        return self.home / "skills"

    @property
    def user_prompts_dir(self) -> Path:
        """Return Pi's user-level prompt templates directory."""
        return self.home / "prompts"

    @property
    def user_themes_dir(self) -> Path:
        """Return Pi's user-level TUI themes directory."""
        return self.home / "themes"

    @property
    def user_extensions_dir(self) -> Path:
        """Return Pi's user-level extension directory."""
    ...
    def project_pi_agent_dir(self, cwd: Path) -> Path:
        """Return the project-local Pi resource directory."""
        return cwd / ".pi-agent"
    ...
    def project_skills_dir(self, cwd: Path) -> Path:
        """Return the project-local Pi skills directory."""
        return self.project_pi_agent_dir(cwd) / "skills"

    def project_prompts_dir(self, cwd: Path) -> Path:
        """Return the project-local Pi prompt templates directory."""
        return self.project_pi_agent_dir(cwd) / "prompts"

    def project_themes_dir(self, cwd: Path) -> Path:
        """Return the project-local Pi TUI themes directory."""
        return self.project_pi_agent_dir(cwd) / "themes"
