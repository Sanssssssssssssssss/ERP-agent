"""Install Pi extensions from local paths or Git repositories."""
...
def install_extension(
    ...
    *,
    ...
    """Install one extension into Pi's user extension directory.

    Local Python files are copied. Local package directories and Git
    repositories must expose ``extension.py`` or ``[tool.pi-agent].extensions`` so
    the normal user-directory discovery path can load them on the next run.
    """
    destination_root = extensions_dir or PiPaths().user_extensions_dir
    ...
    try:
        ...
        # Reproduce the exact ~/.pi-agent/extensions/<name> shape during validation.
...
def parse_git_extension_source(source: str) -> GitExtensionSource:
    """Normalize a Pi-style Git source accepted by ``pi-agent install``."""
...
def _validate_staged_extension(staging_root: Path) -> None:
    paths = PiResourcePaths(root=staging_root, cwd=Path.cwd())
    ...
    if not discovered:
        raise ExtensionInstallError(
            ...
            "[tool.pi-agent].extensions in pyproject.toml"
        )
