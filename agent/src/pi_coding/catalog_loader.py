...
from pi_coding.thinking import ThinkingLevel, ThinkingParameter
...
def builtin_catalog_resource_text() -> str:
    ...
    return files("pi_coding").joinpath("data/catalog.toml").read_text(encoding="utf-8")
...
def user_catalog_path(paths: PiPaths | None = None) -> Path:
    """Return the user-level catalog overlay path."""
    return (paths or PiPaths()).home / USER_CATALOG_FILENAME


def effective_catalog(paths: PiPaths | None = None) -> tuple[ProviderCatalogEntry, ...]:
...
def _builtin_raw_with_cached_models(paths: PiPaths | None) -> dict[str, Any]:
    from pi_coding.models_dev_store import cached_models_dev_catalog_overlay
...
def _builtin_raw_with_generated_models() -> dict[str, Any]:
    ...
    from pi_coding.models_dev import bundled_models_dev_catalog_overlay
