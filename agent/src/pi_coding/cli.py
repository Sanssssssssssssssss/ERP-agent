"""Command-line entry point for Pi Agent for Python."""
...
from pi_agent.session import JsonlSessionStorage, SessionEntry, SessionStorage
...
from pi_coding.commands import format_reload_summary
from pi_coding.credentials import FileCredentialStore
from pi_coding.extension_installer import ExtensionInstallError, install_extension
from pi_coding.extensions import StderrUiBridge
from pi_coding.models_dev_store import (
    ModelsDevRefreshError,
    ModelsDevRefreshResult,
    refresh_models_dev_catalog,
)
from pi_coding.project_trust import TrustDefault, TrustOverride
...
from pi_coding.rendering import PrintOutputMode, create_event_renderer
from pi_coding.resources import PiResourcePaths
from pi_coding.rpc import RpcServer
...
from pi_coding.session_manager import CodingSessionRecord, SessionManager, validate_session_id
from pi_coding.session_preparation import prepare_coding_session
from pi_coding.shell_config import load_shell_settings
from pi_coding.thinking import THINKING_LEVELS, ThinkingLevel, normalize_thinking_level
from pi_coding.tui import run_tui_app
from pi_coding.update_check import (
    UpdateNotice,
    startup_release_notes_notice,
    startup_update_notice,
)
from pi_coding.updater import update_pi_agent
from pi_coding.version import current_version as _current_version
...
app = typer.Typer(
    name="pi-agent",
    help="Pi Agent for Python.",
    epilog="""Commands:

  pi-agent install SOURCE [--force] - Install a trusted local or Git extension.

  pi-agent update - Upgrade Pi Agent.

  pi-agent sessions - List indexed sessions.

  pi-agent export REF [DEST] - Export a session as HTML or JSONL.

  pi-agent providers - List configured model providers.

  pi-agent setup - Configure an OpenAI-compatible provider.
""",
...
def install_command(args: list[str]) -> None:
    """Install an extension into Pi Agent's user extension directory."""
    ...
    for arg in args:
        if arg == "--force":
            ...
            raise typer.BadParameter(f"Unknown option for `pi-agent install`: {arg}")
        elif source is None:
            source = arg
        else:
            raise typer.BadParameter("Usage: pi-agent install <source> [--force]")
    if source is None:
        raise typer.BadParameter("Usage: pi-agent install <source> [--force]")
...
def setup_command(
    *,
    ...
    if provider.api_key_env not in environ:
        typer.echo(
            f"Set {provider.api_key_env} before running Pi Agent with this provider.", err=True
        )
...
def main(
    ...
    setup_base_url: Annotated[
        str,
        typer.Option("--base-url", help="OpenAI-compatible base URL for `pi-agent setup`."),
    ] = DEFAULT_OPENAI_COMPATIBLE_BASE_URL,
    setup_api_key_env: Annotated[
        str,
        typer.Option("--api-key-env", help="API key environment variable for `pi-agent setup`."),
    ] = "OPENAI_API_KEY",
    setup_timeout_seconds: Annotated[
        float,
        typer.Option(
            "--timeout-seconds",
            help="HTTP timeout in seconds for `pi-agent setup` provider requests.",
        ),
    ] = DEFAULT_OPENAI_COMPATIBLE_TIMEOUT_SECONDS,
    setup_max_retries: Annotated[
        int,
        typer.Option("--max-retries", help="Provider retry count for `pi-agent setup`."),
    ] = DEFAULT_OPENAI_COMPATIBLE_MAX_RETRIES,
    setup_max_retry_delay_seconds: Annotated[
        float,
        typer.Option(
            "--max-retry-delay-seconds",
            help="Provider retry delay in seconds for `pi-agent setup`.",
        ),
    ] = DEFAULT_OPENAI_COMPATIBLE_MAX_RETRY_DELAY_SECONDS,
    ...
    export: Annotated[
        ...
        typer.Option(
            "--export",
            help="Export the given session id or JSONL path (mirrors `pi-agent export`).",
        ),
    ] = False,
    ...
    project_extensions: Annotated[
        ...
        typer.Option(
            "--project-extensions",
            help="Also load trusted project .pi-agent/extensions (additional code opt-in).",
        ),
    ] = False,
    approve: Annotated[
        bool,
    ...
    version: Annotated[
        bool,
        typer.Option("--version", "-v", help="Show Pi Agent's version and exit."),
    ] = False,
    models: Annotated[
        bool,
        typer.Option("--models", help="With `pi-agent update`, refresh model catalogs only."),
    ] = False,
) -> None:
    """Run the Pi Agent CLI."""
    current_version = _current_version()
    if version:
        typer.echo(f"pi-agent {current_version}")
        raise typer.Exit()
    ...
    if resume is not None:
        raise typer.BadParameter(
            f"--resume was renamed to --session. Use `pi-agent --session {resume}` instead."
        )
    ...
    if prompt_option is not None:
        raise typer.BadParameter(
            ...
            f'`pi-agent --print "{prompt_option}"`.'
        )

    if output is not None:
        raise typer.BadParameter(
            f"--output was renamed to --mode. Use `pi-agent --mode {output.value}` instead."
        )
    ...
    if not rpc_requested and not print_requested and not export and command == "update":
        ...
        if len(positional_args) != 1 and not positional_models:
            raise typer.BadParameter("Usage: pi-agent update [--models]")
    ...
    if models:
        raise typer.BadParameter("--models is only supported with `pi-agent update`")
    ...
    if not print_requested:
        ...
        if resumable_session_id is not None:
            typer.echo(f"To resume this session: pi-agent --session {resumable_session_id}")
        raise typer.Exit()

    prompt = _merge_stdin_prompt(initial_prompt or "")
    if not prompt:
        raise typer.BadParameter(
            'Usage: pi-agent --print "<prompt>" (or --mode text|json|transcript "<prompt>"); '
...
def update_command() -> None:
    """Upgrade Pi Agent using the installer that manages the current environment."""
    result = update_pi_agent()
    if not result.succeeded:
        typer.echo("Could not safely update Pi Agent Agent:", err=True)
    ...
    if result.deferred:
        typer.echo(f"Pi Agent update handed off with: {' '.join(result.command or ())}")
    else:
        typer.echo(f"Pi Agent update completed with: {' '.join(result.command or ())}")
...
def _run_export_cli(args: list[str]) -> None:
    """Run `pi-agent export`/`pi-agent --export` and exit."""
    try:
...
def _merge_stdin_prompt(prompt: str) -> str:
    ...
    When stdin is not a terminal (e.g. `cat file | pi-agent -p "..."`), its
...
def _parse_export_cli_args(args: list[str]) -> tuple[str, Path | None, str | None]:
    if not args:
        raise RuntimeError(
            "Usage: pi-agent export <session-id-or-jsonl> [--format html|jsonl] [output]"
        )
    session_ref = args[0]
    output_path: Path | None = None
    export_format: str | None = None
...
def _resolve_export_source(
    ...
    if candidate_path.exists():
        ...
        return candidate_path, f"Pi Agent session {candidate_path.stem}"
    ...
    title = record.title or f"Pi Agent session {record.id}"
