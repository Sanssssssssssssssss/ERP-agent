...
from pi_agent.messages import TextContent, UserMessage
...
from pi_agent.tools import AgentTool, AgentToolResult, ToolCancellationToken
from pi_agent.types import JSONValue
from pi_ai.env import DEFAULT_OPENAI_COMPATIBLE_TIMEOUT_SECONDS, OpenAICompatibleConfig
...
class LlamaCppService:
    ...
    def __init__(
        ...
        *,
        ...
        self.credential_store: CredentialStore = (
            ...
            else FileCredentialStore(credentials_path(PiPaths()))
        )
    ...
    async def status(self, context: LocalOperationContext) -> LocalBackendStatus:
        ...
        if not self.configured:
            ...
            return self._status(
                ...
                diagnostics=(
                    LocalDiagnostic(
                        "Pi will check the offered default endpoint when this backend opens. "
        ...
        results = tuple(
            LocalSearchResult(
                ...
                diagnostic=(
                    ...
                    "Pi's search token is never forwarded."
                    if repository.gated
                    else None
                ),
            )
            ...
            return LocalOperationResult(
                ...
                diagnostics=(
                    LocalDiagnostic(
                        ...
                        "Refresh before retrying; Pi will not replay the mutation.",
                        "warning",
                        "reconciliation",
                    ),
                ),
            ...
            return LocalOperationResult(
                ...
                diagnostics=(
                    LocalDiagnostic(
                        ...
                        "retrying; Pi will not replay the interrupted mutation.",
                        "error",
                        "reconciliation",
                    ),
                ),
    ...
    async def _probe_tools(
        ...
        try:
            async for event in provider.stream_response(
                model=model,
                system="Call the pi_probe tool.",
                messages=[UserMessage(content="Call pi_probe now.")],
                tools=[tool],
                signal=signal,
            ):
...
def _reported_context_window(item: Mapping[str, object]) -> int | None:
    # These names are accepted only when directly reported by a server. Pi
