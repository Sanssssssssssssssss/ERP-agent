...
from pi_coding.built_in_extensions import BuiltInExtensionContext
...
def setup(
    ...
    dependencies = context or BuiltInExtensionContext(
        paths=PiPaths(),
        credential_store=FileCredentialStore(credentials_path(PiPaths())),
    ...
    service = LlamaCppService(
        ...
        register_provider=pi.register_provider,
        update_provider=pi.update_provider,
        register_backend=pi.register_local_backend,
    )
    pi.register_provider(service.provider())
    pi.register_local_backend(service.backend())
