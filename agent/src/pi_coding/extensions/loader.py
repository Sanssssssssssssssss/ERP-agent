...
def extension_dirs(
    ...
    *,
    ...
    Project extensions are opt-in (`--project-extensions`) until Pi has a
    ...
    if include_project_dir and paths.cwd is not None and paths.project_resources_enabled:
        dirs.append(paths.cwd / ".pi-agent" / "extensions")
    if include_user_dir:
...
def discover_extensions(
    ...
    *,
    ...
    if include_resource_dirs:
        ...
        if include_project_dir and paths.cwd is not None and paths.project_resources_enabled:
            directories.append((paths.cwd / ".pi-agent" / "extensions", "project"))
        if include_user_dir:
...
def _manifest_entries(
    ...
    """Resolve entry files declared in `<dir>/pyproject.toml` under `[tool.pi-agent]`.
    ...
    use package.json manifest"); `tool.pi-agent.extensions` is the Python-shaped
    ...
    pi_table = tool_table.get("pi-agent") if isinstance(tool_table, dict) else None
    declared = pi_table.get("extensions") if isinstance(pi_table, dict) else None
    if declared is None:
        return []
    if not isinstance(declared, list) or not all(isinstance(item, str) for item in declared):
        diagnostics.append(
            ResourceDiagnostic(
                kind="extension",
                path=manifest,
                message="`tool.pi-agent.extensions` must be a list of file paths",
                severity="error",
            )
        )
        return []
    ...
    if not callable(setup):
        ...
        return None, [
            _error_diagnostic(
                ...
                f"`{EXTENSION_ENTRY_ATTRIBUTE}(pi)` entry point",
            )
        ]
