...
from pi_coding.prompt_templates import is_prompt_template_candidate
from pi_coding.skills import is_skill_candidate
...
def _darwin_filesystem_path(path: Path) -> Path:
    ...
    try:
        raw = fcntl_api.fcntl(descriptor, 50, b"\0" * 1024)  # F_GETPATH
    finally:
        os.close(descriptor)
...
class ProtectedResourceDetector:
    ...
    def detect(self, cwd: CanonicalProjectPath) -> ProtectedResourceSummary:
        ...
        # Project settings are not supported by a Pi loader, so they cannot
        # trigger trust until that loader exists.
        for namespace in (".pi-agent", ".agents"):
            self._glob(
                found,
                "skills",
            ...
            self._glob(
                ...
                root / namespace / "skills",
                "*.md",
                predicate=lambda path: path.name != "SKILL.md" and is_skill_candidate(path),
            )
            self._glob(
                found,
                "prompts",
        ...
        self._glob(found, "themes", root / ".pi-agent" / "themes", "*.json")
        self._file(found, "system-prompts", root / ".pi-agent" / "SYSTEM.md")
        self._file(found, "system-prompts", root / ".pi-agent" / "APPEND_SYSTEM.md")
        self._context(found, root)
        self._extensions(found, root / ".pi-agent" / "extensions")
        counts = {
    ...
    def _context(self, found: dict[str, list[Path]], cwd: Path) -> None:
        # Match current Pi discovery: nearest project marker through cwd, then
        ...
        for part in relative.parts:
            current /= part
            self._file(found, "context", current / "AGENTS.md")
        self._file(found, "context", cwd / ".pi-agent" / "AGENTS.md")
...
class ProjectTrustStore:
    ...
    def __init__(self, paths: PiPaths | None = None) -> None:
        self.paths = paths or PiPaths()
