# Pi Agent CLI and commands

Pi Agent supports print mode, Pi-compatible JSONL RPC mode, and a Textual interactive
TUI. `pi-agent` opens the TUI by default. Print mode is selected with `-p/--print` or
`--mode` and uses the same staged session/provider preparation as the TUI. The
CLI entry point is `pi_coding.cli:app`.
...
```bash
pi-agent update --models
```

This forces ETag revalidation of models.dev and the live NVIDIA model filter,
then atomically caches the transformed catalog at `~/.pi-agent/models-store.json`.
...
set `PI_AGENT_OFFLINE=1` to disable catalog network access.
