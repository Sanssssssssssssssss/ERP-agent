...
def load_prompt_templates(paths: PiResourcePaths | None = None) -> list[PromptTemplate]:
    """Load markdown prompt templates from Pi and `.agents` resource directories."""
    resource_paths = paths or PiResourcePaths()
...
def load_prompt_templates_with_diagnostics(
    paths: PiResourcePaths | None = None,
            ...
            if previous is not None:
                diagnostics.append(
                    ResourceDiagnostic(
                        ...
                        name=template.name,
                        path=template.path,
                        message=f"ignored because the first resource wins at {previous.path}",
                    )
                )
                continue
