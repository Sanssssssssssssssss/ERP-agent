"""Durable Textual TUI configuration for Pi."""
...
class TuiConfigError(ValueError):
    """Raised when Pi TUI configuration is invalid."""


@dataclass(frozen=True, slots=True)
class TuiKeybindings:
    """Configurable keys for Pi's built-in Textual frontend."""
...
class TuiSettings:
    ...
    def resolved_theme(self) -> TuiTheme:
        """Return the selected theme, falling back to pi-dark when unknown."""
        try:
...
def tui_settings_path(paths: PiPaths | None = None) -> Path:
    """Return the durable TUI settings path."""
    return (paths or PiPaths()).home / "tui.json"


def load_tui_settings(paths: PiPaths | None = None) -> TuiSettings:
...
def save_tui_settings(settings: TuiSettings, paths: PiPaths | None = None) -> Path:
...
def tui_settings_from_json(data: dict[str, Any]) -> TuiSettings:
    ...
    # Ignore settings added by newer Pi versions so sharing this user-level
    ...
    return TuiSettings(
        ...
        theme=_theme_name(data.get("theme", "pi-dark")),
