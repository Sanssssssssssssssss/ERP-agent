"""Minimal Textual app for Pi coding sessions."""
...
from pi_agent.provider import CancellationToken
...
from pi_coding.catalog_loader import save_user_catalog_entries
...
from pi_coding.oauth import login_openai_codex
from pi_coding.oauth_registry import get_oauth_provider, oauth_provider_ids
from pi_coding.oauth_types import (
    OAuthAuthInfo,
    OAuthDeviceCodeInfo,
    OAuthLoginCallbacks,
    OAuthPrompt,
...
from pi_coding.project_trust import ProjectTrustRequest, TrustChoice, TrustOverride
from pi_coding.prompt_templates import PromptTemplate
from pi_coding.provider_catalog import (
...
from pi_coding.provider_runtime import ClosableModelProvider, create_model_provider
...
from pi_coding.session_manager import CodingSessionRecord, SessionManager
from pi_coding.session_preparation import prepare_coding_session
from pi_coding.shell_config import load_shell_settings
from pi_coding.skills import Skill
from pi_coding.thinking import ThinkingLevel
from pi_coding.tui.adapter import TuiEventAdapter
from pi_coding.tui.autocomplete import (
    CompletionItem,
    CompletionOption,
    CompletionState,
...
from pi_coding.tui.file_drop import normalize_dropped_paths
from pi_coding.tui.local_backends import (
...
from pi_coding.tui.project_trust import ProjectTrustScreen, prompt_project_trust
from pi_coding.tui.state import TuiState, format_terminal_command_result_block
...
_textual_theme_for_pi_theme = textual_theme_for_tui_theme
...
PROMPT_PLACEHOLDER = "Ask Pi Agent…  Enter submits, Shift+Enter inserts a newline"
...
class _TuiExtensionUiBridge:
    ...
    def register_key_interceptor(self, handler: KeyInterceptor) -> Callable[[], None]:
        ...
        ``PiTuiApp.on_event`` before Textual's app-level priority bindings and
        before the focused widget receives the key, so it can own navigation
        keys (``up``/``down``/``tab``/…) that pi-agent otherwise binds with
...
class _MainViewHandle:
    ...
    def __init__(self, app: PiTuiApp, result: asyncio.Future[object | None]) -> None:
        self._app = app
        self._open = True
...
class PromptInput(TextArea):
    ...
    def handle_pasted_text(self, text: str) -> bool:
        """Apply Pi's paste rules to *text*.
    ...
    async def on_key(self, event: Key) -> None:
        ...
        :meth:`PiTuiApp.on_event` (pre-dispatch, before app-level priority
...
class PromptTemplatePickerScreen(ModalScreen[PromptTemplatePickerResult | None]):
    ...
    def action_edit_cursor(self) -> None:
        """Open the selected template in Pi's prompt editor."""
        template = self._selected_template()
        if template is not None:
...
class OAuthLoginScreen(ModalScreen[OAuthCredential | _LoginFlowAction | None]):
    ...
    def _show_auth(self, info: OAuthAuthInfo) -> None:
        ...
        self.call_after_refresh(self.query_one("#login-oauth-code", Input).scroll_visible)
...
#: (Pi binds it to ``clear_prompt``, but it is the terminal-standard
#: SIGINT/interrupt reflex users hit to bail). NOT reserved: escape/enter/
#: arrows/tab/left/right — those are load-bearing for the pi-subagents
#: extension and must stay interceptable. This is Pi's counterpart to Pi's
...
class PiTuiApp(App[None]):
    """Interactive Textual frontend for a ``CodingSession``."""

    TITLE = "Pi Agent for Python"
    CSS = """
    Screen {
        layout: vertical;
        background: $pi-screen-background;
        color: $pi-screen-text;
    }

    Toast {
        background: $pi-chrome-background;
        color: $pi-chrome-text;
    }
    ...
    #sidebar {
        ...
        height: 1fr;
        padding: 1 1 1 2;
        background: $pi-prompt-background;
        border: none;
    }
    ...
    PiTuiApp.-hide-sidebar #sidebar {
        display: none;
    }

    PiTuiApp.-hide-sidebar #main-pane {
        padding-left: 1;
    }

    PiTuiApp.-sidebar-right #sidebar {
        dock: right;
    }
    ...
    #transcript {
        height: 1fr;
        border: none;
        background: $pi-transcript-background;
        padding: 0 0 0 2;
        overflow-x: auto;
    ...
    #prompt:focus {
        border-left: tall $pi-prompt-border;
    }

    #prompt.-shell-mode {
        border-left: tall $pi-tool-running;
    }
    ...
    #autocomplete {
        ...
        margin: 0 1 1 1;
        padding: 0 1;
        background: $pi-autocomplete-background;
        color: $pi-screen-text;
        border: tall $pi-border;
        overflow-y: auto;
    }
    ...
    ListView > ListItem.-highlight {
        background: $pi-highlight-background;
        color: $pi-highlight-text;
    }
    ...
    ProjectTrustScreen {
        background: $pi-screen-background 60%;
    }
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self._register_pi_textual_themes()
    ...
    def _register_pi_textual_themes(self) -> None:
        """Register Pi themes with Textual's theme system.

        Textual exposes its own theme menu and command palette entries. Registering
        Pi's built-in themes there makes those controls update the same theme as
        `/theme` instead of changing only Textual's chrome.
        """
        self._registered_themes.clear()
        for theme_name in available_tui_theme_names():
            self.register_theme(_textual_theme_for_pi_theme(theme_name))
    ...
    def _watch_theme(self, theme_name: str) -> None:
        """Keep Textual theme changes synchronized with Pi's durable TUI theme."""
        super()._watch_theme(theme_name)
        if theme_name not in available_tui_theme_names():
            return
        if getattr(self, "_applying_settings_theme", False):
            return
        pi_theme: TuiThemeName = theme_name
        if self.tui_settings.theme == pi_theme:
            return
        self._replace_tui_settings(theme=pi_theme)
        save_tui_settings(self.tui_settings)

    def get_theme_variable_defaults(self) -> dict[str, str]:
        """Return Pi-specific CSS variables for the selected TUI theme."""
        ...
        with Horizontal(id="workspace"):
            ...
            with Vertical(id="main-pane"):
                ...
                with Horizontal(id="prompt-row"):
                    yield Static("π", id="prompt-prefix")
                    yield PromptInput(
    ...
    async def on_event(self, event: events.Event) -> None:
        ...
        the earliest point in key processing — before pi's app-level priority
    ...
    def on_app_focus(self) -> None:
        """Suppress turn notifications while the Pi terminal surface is active."""
        self._app_has_focus = True
    ...
    def action_open_session_picker(self) -> None:
        ...
        if self.state.running:
            self._notify("Pi Agent is already working. Press Escape to cancel.")
            return
    ...
    async def _use_local_model(self, provider_id: str, model_id: str) -> None:
        if self._is_agent_or_queue_active():
            self._notify(
                "Pi Agent is still working. Press Escape to interrupt before switching models.",
                severity="warning",
            )
            return
...
def _render_activity_indicator(
    ...
    *,
    ...
    if not running:
        return Text("π", style=f"bold {theme.accent}")
...
def _attach_retry_hint_to_error(state: TuiState, message: AssistantMessage) -> None:
    ...
    they are skipped to avoid asking the user to retry while Pi already is.
    """
