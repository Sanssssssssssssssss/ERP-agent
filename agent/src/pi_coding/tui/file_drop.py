...
def normalize_dropped_paths(text: str) -> str | None:
    ...
    # On Windows, a terminal may still shell-escape those spaces even though
    # backslashes are also path separators.
    whole = _token_to_path(stripped.replace("\\ ", " "))
    ...
    try:
        ...
        if os.name == "nt":
            escaped_space = "\0"
            shell_text = (
                shell_text.replace("\\ ", escaped_space)
                .replace("\\", "/")
                .replace(escaped_space, "\\ ")
            )
        tokens = shlex.split(shell_text, posix=True)
    except ValueError:
        return None
    if not tokens:
        return None
...
def _token_to_path(token: str) -> str | None:
    ...
    if candidate.startswith("file://"):
        ...
        if os.name == "nt" and not parsed.path and len(parsed.netloc) > 1:
            candidate = unquote(parsed.netloc)
        else:
