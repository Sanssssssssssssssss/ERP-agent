...
``~/.pi-agent/themes/*.json`` and ``<project>/.pi-agent/themes/*.json``.
"""
...
PI_DARK_THEME = _load_builtin_theme("pi-dark.json")
PI_LIGHT_THEME = _load_builtin_theme("pi-light.json")
HIGH_CONTRAST_THEME = _load_builtin_theme("high-contrast.json")

_BUILTIN_THEMES: dict[str, TuiTheme] = {
    theme.name: theme for theme in (PI_DARK_THEME, PI_LIGHT_THEME, HIGH_CONTRAST_THEME)
}
...
def get_tui_theme(name: TuiThemeName = "pi-dark") -> TuiTheme:
