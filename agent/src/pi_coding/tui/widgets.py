"""Small Textual widgets for Pi's interactive TUI."""
...
from pi_coding.context_window import estimate_text_tokens
...
from pi_coding.system_prompt import ProjectContextFile, format_skills_for_prompt
from pi_coding.tui.autocomplete import CompletionState
from pi_coding.tui.config import PI_DARK_THEME, TuiRoleStyle, TuiTheme
from pi_coding.tui.state import (
...
from pi_coding.version import current_version

PI_SIDEBAR_LOGO = "π  Pi Agent"
...
class SessionSidebar(Vertical):
    ...
    def compose(self) -> Any:
        with VerticalScroll(id="sidebar-scroll"):
            ...
            yield Static(_sidebar_separator(theme=PI_DARK_THEME), classes="sidebar-separator")
            yield Collapsible(
            ...
            yield Collapsible(
                ...
                title=_sidebar_resource_title("prompts", "0", theme=PI_DARK_THEME),
                collapsed=True,
                id="sidebar-prompts",
                classes="sidebar-resource-section",
            )
...
class PiMarkdownBlock(MarkdownBlock):
    """Markdown block that applies Pi's themed inline link color."""
    ...
    def _token_to_content(self, token: Any) -> Any:
        ...
        if not isinstance(markdown, ThemedMarkdownWidget):
            return content
        link_style = TextualStyle.parse(markdown.pi_link_style)
        spans = []
...
class PiMarkdownFence(MarkdownFence):
...
class ThemedMarkdownWidget(TextualMarkdown):
    """Textual Markdown widget reserved for Pi transcript streaming."""

    BLOCKS = {
        **TextualMarkdown.BLOCKS,
        "paragraph_open": PiMarkdownBlock,
        "fence": PiMarkdownFence,
        "code_block": PiMarkdownFence,
    }
    ...
    ThemedMarkdownWidget MarkdownH6 {
        color: $pi-markdown-highlight;
        content-align: left middle;
        text-style: bold;
    }

    ThemedMarkdownWidget MarkdownBlock > .code_inline {
        color: $pi-markdown-inline-code !important;
        background: transparent !important;
    }

    ThemedMarkdownWidget MarkdownBullet {
        color: $pi-markdown-bullet;
    }

    ThemedMarkdownWidget MarkdownFence {
        background: $pi-markdown-code-block-background;
        overflow-x: auto;
    ...
    ThemedMarkdownWidget MarkdownTableContent {
        keyline: thin $pi-markdown-table-border;
    }

    ThemedMarkdownWidget MarkdownTableContent > .header {
        color: $pi-markdown-table-header;
        text-style: bold;
    }
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.pi_link_style = theme.markdown_link
...
class TranscriptView(VerticalScroll):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self._render_theme: TuiTheme = PI_DARK_THEME
...
class ThemedCodeBlock(CodeBlock):
    """Rich Markdown code block with Pi's themed background color."""
...
class ThemedMarkdown(Markdown):
    """Markdown renderer with Pi's softer heading/accent colors."""
...
def _grouped_resource_names(
    ...
    *,
    ...
    origin_precedence = {
        f"~/.pi-agent/{directory}": 0,
        f"~/.agents/{directory}": 1,
        f"./.pi-agent/{directory}": 2,
