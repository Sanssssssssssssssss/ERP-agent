...
class LocalBackendScreen(ModalScreen[None]):
    ...
    def _confirm_reset(self) -> None:
        if not self._is_idle():
            self._show_message(
                "Pi must be idle before resetting local backend settings.",
                "warning",
            )
            return
        self.app.push_screen(
    ...
    def _open_model_action(self, action: LocalAction) -> None:
        if not self._is_idle():
            self._show_message(
                "Pi must be idle before changing local backend models.",
                "warning",
            )
            return
        labels = {
    ...
    def _start_operation(
        ...
        *,
        ...
        if not self._is_idle():
            self._show_message(
                "Pi must be idle before changing local backend settings.",
                "warning",
            )
            return
        if self._worker is not None and not self._worker.done():
    ...
    def _use_model(self, model_id: str) -> None:
        if not self._is_idle():
            self._show_message("Pi must be idle before switching models.", "warning")
            return
        if self._worker is not None and not self._worker.done():
            self._show_message("An operation is already in progress.", "warning")
            return
