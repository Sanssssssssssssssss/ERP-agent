"""Accessible Textual adapter for Pi-owned project-trust requests."""
...
from pi_coding.tui.themes import PI_DARK_THEME, textual_theme_for_tui_theme
...
class ProjectTrustScreen(ModalScreen[TrustChoice | None]):
    """Pi-style modal picker rendering one policy-owned request."""
    ...
    def on_key(self, event: Key) -> None:
        """Keep navigation local when hosted by Pi's globally bound app."""
        if event.key == "up":
            event.stop()
...
class _ProjectTrustApp(App[TrustChoice | None]):
    def __init__(self, request: ProjectTrustRequest) -> None:
        super().__init__()
        pi_dark = textual_theme_for_tui_theme(PI_DARK_THEME.name)
        self.register_theme(pi_dark)
        self.theme = pi_dark.name
        self.request = request

    def on_mount(self) -> None:
        self.push_screen(
            ProjectTrustScreen(self.request, cancel_action="exits Pi"),
            self.exit,
        )


async def prompt_project_trust(request: ProjectTrustRequest) -> TrustChoice | None:
    """Run the frontend adapter and return the Pi policy choice."""
