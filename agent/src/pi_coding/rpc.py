"""Pi-compatible JSONL RPC frontend for a Pi coding session."""
...
from pi_agent.messages import AssistantMessage, CustomMessage, UserMessage
...
from pi_coding.commands import CommandRegistry
from pi_coding.events import CodingSessionEvent
    ...
    async def _dispatch(self, command: dict[str, object], tasks: anyio.abc.TaskGroup) -> None:
        ...
        try:
            ...
            if command_type == "abort_bash":
                raise ValueError("abort_bash is not supported by Pi yet")
...
def _entry_wire(entry: SessionEntry, provider_name: str) -> dict[str, JSONValue] | None:
    ...
    if entry.type == "compaction":
        ...
        return {
            **base,
            ...
            "details": {"piReplacedEntryIds": list(entry.replaces_entry_ids)},
        }
    ...
    raise AssertionError(f"Unhandled Pi session entry: {entry.type}")
