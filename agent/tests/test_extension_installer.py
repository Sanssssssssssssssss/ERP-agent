...
from pi_coding.extension_installer import (
    ExtensionInstallError,
    install_extension,
    parse_git_extension_source,
)
from pi_coding.extensions import discover_extensions
...
def test_install_local_extension_file_is_discovered(tmp_path: Path) -> None:
    ...
    source.write_text("def setup(pi):\n    pass\n", encoding="utf-8")
    ...
    paths = PiResourcePaths(root=tmp_path / "home", cwd=tmp_path)
...
@pytest.mark.parametrize(
    ...
    [
        (
            "git:github.com/rian-dolphin/pi-subagents@v1.2.0",
            "https://github.com/rian-dolphin/pi-subagents",
            "v1.2.0",
            "pi-subagents",
        ),
        (
            "https://github.com/rian-dolphin/pi-subagents.git",
            "https://github.com/rian-dolphin/pi-subagents.git",
            None,
            "pi-subagents",
        ),
        (
            "git:git@github.com:rian-dolphin/pi-subagents.git@abc123",
            "git@github.com:rian-dolphin/pi-subagents.git",
            "abc123",
            "pi-subagents",
        ),
    ],
...
def test_missing_git_is_reported_as_install_error(tmp_path: Path) -> None:
    ...
    with pytest.raises(ExtensionInstallError, match="git executable not found"):
        install_extension(
            "git:github.com/example/pi-demo",
            extensions_dir=tmp_path / "extensions",
...
def test_install_git_source_clones_and_checks_out_ref(tmp_path: Path) -> None:
    ...
    destination = install_extension(
        "git:github.com/example/pi-demo@v1",
        extensions_dir=tmp_path / "extensions",
        command_runner=fake_run,
    )

    assert destination == tmp_path / "extensions" / "pi-demo"
    assert calls[0][:4] == ["git", "clone", "--", "https://github.com/example/pi-demo"]
