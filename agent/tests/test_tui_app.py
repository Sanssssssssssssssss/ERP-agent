...
from pi_agent.messages import assistant_content
from pi_agent.provider_events import TextDeltaEvent, ThinkingDeltaEvent
from pi_coding.catalog_loader import user_catalog_path
from pi_coding.commands import CommandResult
from pi_coding.credentials import FileCredentialStore, OAuthCredential
...
from pi_coding.extensions import (
    DynamicProvider,
...
from pi_coding.resources import ResourceDiagnostic
from pi_coding.session import (
    ModelChoice,
    SessionTreeBranchResult,
    SessionTreeChoice,
    TerminalCommandResult,
)
from pi_coding.session_manager import CodingSessionRecord
from pi_coding.session_stats import SessionStats
from pi_coding.skills import Skill, format_skill_invocation
from pi_coding.system_prompt import ProjectContextFile
from pi_coding.tools import create_coding_tools
from pi_coding.tui import app as tui_app
from pi_coding.tui.app import (
    ...
    _textual_theme_for_pi_theme,
    _theme_css_variables,
    _TuiExtensionUiBridge,
    _visible_completion_state,
)
from pi_coding.tui.autocomplete import CompletionItem, CompletionState
...
from pi_coding.tui.local_backends import LocalBackendScreen, LocalConfirmScreen
from pi_coding.tui.state import ChatItem, TuiState
from pi_coding.tui.terminal_notification import TerminalNotificationController
from pi_coding.tui.terminal_title import TerminalTitleController
...
class FakeSession:
    def __init__(self, messages=(), events=()) -> None:
        ...
        self.skills = (
            Skill(
                name="review",
                path=self.cwd / ".pi-agent" / "skills" / "review" / "SKILL.md",
                content="Review code",
            ),
        )
...
def _visible_footer_bindings(app: PiTuiApp) -> dict[str, str]:
...
def test_session_sidebar_renders_session_metadata() -> None:
    ...
    assert "π  Pi Agent" not in output
    ...
    assert "read, bash, edit, write" in output
    assert re.search(r"\./\.pi-agent/skills\s+• review", output)
...
def test_session_sidebar_shows_all_skills() -> None:
    ...
    session.skills = tuple(
        Skill(
            name=f"skill-{index}",
            path=session.cwd / ".pi-agent" / "skills" / f"skill-{index}" / "SKILL.md",
            content="Skill",
        )
...
def test_session_sidebar_marks_user_only_skills_with_hollow_bullets() -> None:
    ...
    session.skills = (
        Skill(
            name="model-visible",
            path=session.cwd / ".pi-agent/skills/model-visible/SKILL.md",
            content="Model-visible skill",
        ),
        Skill(
            name="user-only",
            path=session.cwd / ".pi-agent/skills/user-only/SKILL.md",
...
def test_session_sidebar_groups_skills_by_origin(
    ...
    session.skills = (
        ...
        Skill("user-pi", tmp_path / ".pi-agent/skills/user-pi/SKILL.md", ""),
        Skill("project-pi", session.cwd / ".pi-agent/skills/project-pi/SKILL.md", ""),
    ...
    output = console.export_text()
    expected_groups = (
        ("~/.pi-agent/skills", "user-pi"),
        ("~/.agents/skills", "user-agents"),
        ("./.pi-agent/skills", "project-pi"),
...
def test_session_sidebar_groups_and_shows_all_prompts(
    ...
    session.prompt_templates = (
        ...
        PromptTemplate("user-pi", tmp_path / ".pi-agent/prompts/user-pi.md", ""),
        PromptTemplate("project-pi", session.cwd / ".pi-agent/prompts/project-pi.md", ""),
        PromptTemplate("user-agents", tmp_path / ".agents/prompts/user-agents.md", ""),
        *tuple(
            PromptTemplate(
                f"extra-{index}",
                session.cwd / f".pi-agent/prompts/extra-{index}.md",
                "",
            )
    ...
    output = console.export_text()
    expected_groups = (
        ("~/.pi-agent/prompts", "user-pi"),
        ("~/.agents/prompts", "user-agents"),
        ("./.pi-agent/prompts", "extra-1"),
        ("./.agents/prompts", "project-agents"),
    )
    assert all(
        re.search(rf"{re.escape(origin)}\s+• {name}", output) for origin, name in expected_groups
    )
    assert "• project-pi" in output
    for index in range(1, 5):
...
def test_session_sidebar_lists_active_system_prompt_files() -> None:
    ...
    session.system_prompt_files = (
        session.cwd / ".pi-agent" / "SYSTEM.md",
    ...
    assert "• .pi-agent/SYSTEM.md" in output
    assert "• ~/.pi-agent/APPEND_SYSTEM.md" in output
...
def test_comma_list_limits_by_rendered_lines_instead_of_item_count() -> None:
    ...
    narrow_console.print(_comma_list(items, empty="Empty", theme=PI_DARK_THEME))
    ...
    wide_console.print(_comma_list(items, empty="Empty", theme=PI_DARK_THEME))
...
def test_session_sidebar_brand_includes_current_version() -> None:
    ...
    console.print(_sidebar_brand(theme=PI_DARK_THEME))

    assert "π  Pi Agent  0.1.0" in console.export_text()
...
def test_session_sidebar_uses_prominent_title_and_accented_section_headers() -> None:
    ...
    assert str(session_name.renderable.style) == f"bold {PI_DARK_THEME.accent}"
    assert activity_header.left == 1
    assert str(activity_header.renderable.style) == f"bold {PI_DARK_THEME.prompt_text}"
    assert str(activity_content.renderable.style) == PI_DARK_THEME.completion_description
    assert " context" in output
...
def test_compact_session_info_styles_provider_as_metadata() -> None:
    ...
    assert f"\x1b[{_style_color_escape(PI_DARK_THEME.completion_description)}mopenai" in output
    assert f"\x1b[{_style_color_escape(PI_DARK_THEME.prompt_text)}m:fake-model" in output


def test_compact_session_info_styles_parent_path_as_metadata() -> None:
    cwd = _styled_cwd(Path("/workspace/project"), theme=PI_DARK_THEME)

    assert cwd.plain == "/workspace/project (--)"
    assert str(cwd.spans[0].style) == PI_DARK_THEME.completion_description
    assert str(cwd.spans[1].style) == PI_DARK_THEME.prompt_text
    assert str(cwd.spans[2].style) == PI_DARK_THEME.completion_description
...
def test_tui_state_compacts_expanded_skill_messages() -> None:
    skill = Skill(
        name="review",
        path=Path("/workspace/.pi-agent/skills/review.md"),
        content="# Review\nFull noisy instructions.",
                ...
                content=assistant_content(
                    ...
                    [
                        ToolCall(
                            id="call-1",
                            name="read",
                            arguments={"path": "/workspace/.pi-agent/skills/review.md"},
                        )
                    ],
                )
            ),
...
def test_dark_theme_markdown_code_uses_aqua_highlight() -> None:
    console = Console(record=True, width=80)
    console.print(render_chat_item(ChatItem(role="assistant", text="Use `pi` here.")))
...
def test_assistant_markdown_titles_use_highlight_color_and_left_alignment() -> None:
    ...
    assert _style_color_escape(PI_DARK_THEME.markdown_heading) in output
...
def test_dark_theme_markdown_bullets_use_theme_bullet_color() -> None:
    ...
    assert _style_color_escape(PI_DARK_THEME.markdown_bullet) in output
...
def test_markdown_tables_use_highlight_color_for_headers() -> None:
    ...
    assert _style_color_escape(PI_DARK_THEME.accent) in output
...
async def test_textual_markdown_widget_uses_theme_link_style() -> None:
    ...
    async with app.run_test() as pilot:
        ...
        block = app.query_one(PiMarkdownBlock)
    ...
    assert markdown.pi_link_style == PI_DARK_THEME.markdown_link
...
def test_textual_markdown_uses_theme_highlight_and_aqua_inline_code() -> None:
    variables = _theme_css_variables(PI_LIGHT_THEME)

    assert variables["pi-markdown-highlight"] == PI_LIGHT_THEME.markdown_heading
    assert variables["pi-markdown-table-header"] == PI_LIGHT_THEME.markdown_table_header
    assert variables["pi-markdown-table-border"] == PI_LIGHT_THEME.markdown_table_border
    assert variables["pi-markdown-inline-code"] == PI_LIGHT_THEME.markdown_inline_code
    assert (
        variables["pi-markdown-code-block-background"]
        == PI_LIGHT_THEME.markdown_code_block_background
    )
    assert variables["pi-markdown-link"] == PI_LIGHT_THEME.markdown_link
    assert variables["pi-markdown-bullet"] == PI_LIGHT_THEME.markdown_bullet
...
def test_light_theme_markdown_code_uses_aqua_without_background() -> None:
    ...
    console.print(
        render_chat_item(
            ChatItem(role="assistant", text="Use `pi` here."),
            theme=PI_LIGHT_THEME,
        )
    )
...
def test_expanded_tool_invocation_blank_line_stays_separate_from_result() -> None:
    ...
    body = _transcript_plain_body_text(
        ...
        body_style=PI_DARK_THEME.role_styles["tool"].body,
        theme=PI_DARK_THEME,
        show_tool_results=True,
...
def test_partially_completed_read_group_keeps_running_color() -> None:
    ...
    output = console.export_text(styles=True)
    running_color = _style_color_escape(PI_DARK_THEME.role_styles["tool"].border)
...
async def test_streaming_transcript_applies_role_foreground() -> None:
    app = PiTuiApp(FakeSession())

    async with app.run_test(size=(60, 20)) as pilot:
        await pilot.pause()
        transcript = app.query_one("#transcript", TranscriptView)

        thinking_fg, _ = _split_rich_style_colors(PI_DARK_THEME.role_styles["thinking"].body)
        assistant_fg, _ = _split_rich_style_colors(PI_DARK_THEME.role_styles["assistant"].body)

        # Streamed thinking is dimmed immediately, matching the finalized block
        # instead of shifting color on the next redraw.
        await transcript.append_thinking_delta("reasoning", theme=PI_DARK_THEME, show_thinking=True)
        await pilot.pause()
        thinking = next(
        ...
        await transcript.append_assistant_delta("answer", theme=PI_DARK_THEME)
        await pilot.pause()
        assistant = next(
...
async def test_tool_completion_updates_row_without_redrawing_history() -> None:
    app = PiTuiApp(FakeSession(messages=[UserMessage(content="earlier")]))
...
async def test_long_transcript_mounts_bounded_latest_window_and_pages_earlier(
    ...
    from pi_coding.tui import widgets as tui_widgets
...
async def test_tool_transcript_uses_native_markdown_without_custom_selection_painting() -> None:
    app = PiTuiApp(FakeSession(messages=[]))
    item = ChatItem(
        role="tool",
        text="→ read README.md",
        tool_result_text="✓ read\nREADME contents",
...
def test_pi_markdown_block_is_not_selectable_until_mounted() -> None:
    markdown = TextualMarkdown("example")
    block = PiMarkdownBlock(
        markdown,
...
@pytest.mark.anyio
async def test_pi_markdown_block_remains_selectable_after_mount() -> None:
    app = PiTuiApp(
        FakeSession([AssistantMessage(content="Read [docs](https://example.com).")]),
    )
...
async def test_tui_app_highlights_prompt_shell_mode() -> None:
    ...
    async with app.run_test(size=(120, 30)) as pilot:
        ...
        assert indicator.render().plain == "π"
...
async def test_tui_sidebar_is_visible_on_medium_windows() -> None:
    ...
    async with app.run_test(size=(120, 40)):
        ...
        assert sidebar.styles.background == Color.parse(PI_DARK_THEME.prompt_background)
...
async def test_tui_sidebar_resource_sections_expand_independently() -> None:
    session = FakeSession()
    session.prompt_templates = (
        PromptTemplate("explain", session.cwd / ".pi-agent/prompts/explain.md", "Explain"),
        PromptTemplate("fix", session.cwd / ".pi-agent/prompts/fix.md", "Fix"),
    )
    app = PiTuiApp(session)
    ...
    async with app.run_test(size=(120, 40)) as pilot:
        ...
        assert str(skills_heading.spans[0].style) == f"bold {PI_DARK_THEME.prompt_text}"
        assert str(skills_heading.spans[1].style) == PI_DARK_THEME.completion_description
        assert str(prompts_heading.spans[0].style) == f"bold {PI_DARK_THEME.prompt_text}"
        assert str(prompts_heading.spans[1].style) == PI_DARK_THEME.completion_description
...
async def test_tui_sidebar_shows_on_left_when_configured() -> None:
    app = PiTuiApp(FakeSession(), tui_settings=TuiSettings(sidebar_position="left"))
...
async def test_tui_sidebar_is_hidden_when_off() -> None:
    app = PiTuiApp(FakeSession(), tui_settings=TuiSettings(sidebar_position="off"))
...
async def test_tui_sidebar_right_still_hides_on_small_windows() -> None:
    app = PiTuiApp(FakeSession(), tui_settings=TuiSettings(sidebar_position="right"))
...
async def test_tui_transcript_code_block_scrollbar_matches_overflow(
    ...
    app = PiTuiApp(FakeSession(messages=[AssistantMessage(content=f"```python\n{code}\n```")]))
...
@pytest.mark.parametrize(
    "theme",
    [PI_DARK_THEME, PI_LIGHT_THEME, HIGH_CONTRAST_THEME],
    ids=lambda theme: theme.name,
)
async def test_transcript_code_fence_uses_theme_background(theme: TuiTheme) -> None:
    """Code fences keep Pi's themed background for every built-in theme.

    Regression test: Textual's built-in `MarkdownFence:light` rule outranks
    Pi's plain `ThemedMarkdownWidget MarkdownFence` selector, which used to
...
def test_tui_app_uses_configured_theme_css_variables() -> None:
    app = PiTuiApp(FakeSession(), tui_settings=TuiSettings(theme="high-contrast"))

    variables = app.get_theme_variable_defaults()

    assert variables["pi-screen-background"] == "#000000"
    assert variables["pi-prompt-background"] == "#1a1a1a"
    assert variables["pi-prompt-border"] == "#00ff66"
...
def test_tui_app_registers_and_applies_custom_theme() -> None:
    from pi_coding.tui.themes import (
        THEME_COLOR_FIELDS,
        TRANSCRIPT_ROLES,
    ...
    try:
        app = PiTuiApp(FakeSession(), tui_settings=TuiSettings(theme="midnight"))

        assert app.theme == "midnight"
        assert app.get_theme_variable_defaults()["pi-screen-background"] == "#123456"
    finally:
        set_custom_tui_themes({})
...
async def test_tui_app_removes_source_project_themes_after_resume(
    ...
    session = FakeSession()
    session.theme_dirs = ()
    app = PiTuiApp(session, tui_settings=TuiSettings(theme="project-theme"))
    try:
        async with app.run_test() as pilot:
            await app._resume_session("destination-session")
            await pilot.pause()

            assert app.theme == "pi-dark"
...
def test_tui_app_falls_back_to_pi_dark_when_theme_is_missing(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    isolate_home(monkeypatch, tmp_path)

    app = PiTuiApp(FakeSession(), tui_settings=TuiSettings(theme="missing-theme"))
...
def test_tui_app_uses_light_theme_css_variables() -> None:
    app = PiTuiApp(FakeSession(), tui_settings=TuiSettings(theme="pi-light"))

    variables = app.get_theme_variable_defaults()

    assert variables["pi-screen-background"] == "#ffffff"
    assert variables["pi-chrome-background"] == "#f3f4f6"
    assert variables["pi-muted-text"] == "#475569"
    assert variables["pi-prompt-background"] == "#f8fafc"
    assert variables["pi-prompt-border"] == "#2563eb"
...
def test_tui_app_registers_only_pi_themes_with_textual() -> None:
    app = PiTuiApp(FakeSession())

    assert tuple(app.available_themes) == ("pi-dark", "pi-light", "high-contrast")


def test_textual_theme_mapping_uses_pi_theme_values() -> None:
    textual_theme = _textual_theme_for_pi_theme("pi-light")

    assert textual_theme.name == "pi-light"
    assert textual_theme.primary == PI_LIGHT_THEME.accent
    assert textual_theme.dark is False
    assert textual_theme.variables["pi-screen-background"] == PI_LIGHT_THEME.screen_background


def test_pi_dark_theme_uses_aqua_as_its_shared_accent() -> None:
...
def test_autocomplete_and_picker_share_theme_highlight(theme: TuiTheme) -> None:
    ...
    assert variables["pi-highlight-background"] == theme.highlight_background
    assert variables["pi-highlight-text"] == theme.highlight_text


def test_pi_light_theme_uses_light_chat_backgrounds() -> None:
    theme = TuiSettings(theme="pi-light").resolved_theme
...
async def test_tui_app_updates_terminal_title_for_running_and_named_session() -> None:
    ...
    async with app.run_test():
        assert writes[-1] == "\x1b]0;π | build notes\x07"
        ...
        app._refresh_chrome()
        assert writes[-1] == "\x1b]0;⠙ π | ship notes\x07"

        app.adapter.apply(AgentEndEvent())
        app._refresh()
        assert writes[-1] == "\x1b]0;π | ship notes\x07"

    assert writes[-1] == "\x1b]0;π\x07"
...
async def test_tui_app_notifies_when_agent_settles_while_unfocused() -> None:
    ...
    app = PiTuiApp(SettledSession(), tui_settings=TuiSettings(turn_notification="desktop"))
    writes: list[str] = []
    ...
    async with app.run_test():
        ...
        assert writes == ["\x1b]9;Pi turn finished\x07"]
...
async def test_tui_app_updates_terminal_title_after_auto_session_naming() -> None:
    ...
    app = PiTuiApp(AutoNamingSession())
    writes: list[str] = []
    app._terminal_title = TerminalTitleController(enabled=True, writer=writes.append)
    ...
    async with app.run_test():
        ...
        assert "\x1b]0;π | Debug login\x07" in writes
        ...
        assert writes[-1] == "\x1b]0;π | Debug login\x07"
...
@pytest.mark.anyio
async def test_textual_theme_change_persists_pi_theme(
    ...
    assert app.tui_settings.theme == "pi-light"
    assert tui_settings_path().read_text(encoding="utf-8").find('"theme": "pi-light"') != -1
...
async def test_tui_app_theme_command_opens_picker_and_persists_selection(
    ...
    async with app.run_test() as pilot:
        ...
        assert app.tui_settings.theme == "pi-light"
        assert app.theme == "pi-light"
        assert tui_settings_path().read_text(encoding="utf-8").find('"theme": "pi-light"') != -1
        assert app.get_theme_variable_defaults()["pi-screen-background"] == "#ffffff"
...
async def test_theme_picker_highlight_uses_theme_selection_palette(theme: TuiTheme) -> None:
    theme_names = (PI_DARK_THEME.name, PI_LIGHT_THEME.name, HIGH_CONTRAST_THEME.name)
    app = PiTuiApp(FakeSession(), tui_settings=TuiSettings(theme=theme.name))
...
async def test_extension_select_dialog_returns_choice() -> None:
    app = PiTuiApp(FakeSession())  # type: ignore[arg-type]
...
async def test_local_modals_receive_app_level_arrow_navigation() -> None:
    ...
    async with app.run_test() as pilot:
        ...
        app.push_screen(
            LocalConfirmScreen("Load model?", "This is expensive.", theme=PI_DARK_THEME),
...
async def test_restored_tool_calls_render_via_render_call() -> None:
    ...
    app = PiTuiApp(session)  # type: ignore[arg-type]
...
async def test_tui_app_shows_working_state_during_manual_compaction() -> None:
    ...
    async with app.run_test() as pilot:
        ...
        assert indicator.render().plain != "π"
        assert titles[-1] == "\x1b]0;⠋ π | build notes\x07"
...
async def test_tui_app_clears_working_state_when_manual_compaction_fails() -> None:
    ...
    app = PiTuiApp(FailingCompactSession(messages=[UserMessage(content="Earlier")]))
    notifications: list[str] = []
...
async def test_tui_app_clears_working_state_when_compaction_is_cancelled() -> None:
    ...
    app = PiTuiApp(SlowCompactSession(messages=[UserMessage(content="Earlier")]))
...
async def test_tui_app_notifies_once_for_turn_with_automatic_compaction() -> None:
    ...
    app = PiTuiApp(AutoCompactSession(), tui_settings=TuiSettings(turn_notification="desktop"))
    writes: list[str] = []
    app._terminal_notification = TerminalNotificationController(
        "desktop",
        enabled=True,
...
async def test_tui_app_export_command_runs_session_export() -> None:
    ...
    async with app.run_test() as pilot:
        ...
        assert app.state.items[-1] == ChatItem(
            role="status",
            text=f"/export\nExported session to {session.cwd / 'session.html'}",
        )
        assert session.prompt_texts == []
...
async def test_tui_app_blocks_tree_picker_while_agent_is_running() -> None:
    ...
    async with app.run_test() as pilot:
        ...
        assert notifications == [
            "Pi is still working. Press Escape to interrupt before using /tree."
        ]
...
async def test_tui_app_tree_picker_branches_with_summary() -> None:
    ...
    async with app.run_test() as pilot:
        ...
        assert str(rendered_labels[0].spans[0].style) == _style_rgb(PI_DARK_THEME.accent)
        assert str(rendered_labels[3].spans[0].style) == _style_rgb(PI_DARK_THEME.highlight_text)
        ...
        assert str(left_label.spans[0].style) == _style_rgb(PI_DARK_THEME.highlight_text)
        assert str(right_label.spans[0].style) == _style_rgb(PI_DARK_THEME.accent)
        await pilot.press("s")
        await pilot.pause()
...
async def test_tui_app_session_modal_auto_copies_selected_text(
    ...
    app = PiTuiApp(FakeSession(), tui_settings=TuiSettings(auto_copy_selection=False))
    copied: list[str] = []
    monkeypatch.setattr(app, "copy_to_clipboard", copied.append)
...
async def test_tui_login_saves_provider_key(
    ...
    assert (tmp_path / ".pi-agent" / "credentials.json").read_text(encoding="utf-8")
...
async def test_tui_login_openai_codex_saves_oauth_credentials(
    ...
    credentials = (tmp_path / ".pi-agent" / "credentials.json").read_text(encoding="utf-8")
...
async def test_tui_login_custom_provider_writes_catalog_and_preferences(
    ...
    paths = PiPaths(home=tmp_path / ".pi-agent")
    ...
    assert FileCredentialStore(tmp_path / ".pi-agent" / "credentials.json").get("nebius") == (
        "stored-nebius-key"
    )
    assert session.provider_reload_count == 1
...
async def test_tui_login_preserves_existing_scoped_models_and_providers(
    ...
    assert FileCredentialStore(pi_home / "credentials.json").get("openrouter") == (
...
async def test_tui_logout_removes_stored_api_key(
    ...
    isolate_home(monkeypatch, tmp_path)
    credential_path = tmp_path / ".pi-agent" / "credentials.json"
...
async def test_tui_logout_opens_stored_credential_provider_picker(
    ...
    isolate_home(monkeypatch, tmp_path)
    FileCredentialStore(tmp_path / ".pi-agent" / "credentials.json").set("anthropic", "stored-key")
    app = PiTuiApp(FakeSession())
...
async def test_tui_app_up_arrow_preserves_non_empty_prompt_movement() -> None:
    app = PiTuiApp(FakeSession(messages=[UserMessage(content="remembered prompt")]))
...
async def test_tui_prompt_ctrl_c_clears_text() -> None:
    app = PiTuiApp(FakeSession(messages=(UserMessage(content="User prompt"),)))
...
async def test_tui_prompt_worker_refreshes_directly() -> None:
    app = PiTuiApp(FakeSession(events=[AgentStartEvent(), AgentEndEvent()]))
    refreshes = 0
...
async def test_tui_prompt_worker_shows_diagnostic_log_path_for_error_event(tmp_path: Path) -> None:
    class ErrorSession(FakeSession):
        def __init__(self) -> None:
            ...
            self.last_diagnostic_log_path = tmp_path / "pi-home" / "logs" / "agent-calls.jsonl"
...
async def test_tui_app_shows_startup_update_notice_first_in_bright_yellow() -> None:
    ...
    app = PiTuiApp(
        session,
        startup_update_notice="Pi 0.2.0 is available",
        startup_alerts=("Conflicting skills/prompts detected",),
        startup_notices=("Pi updated to 0.2.0",),
    )
    notifications: list[tuple[str, str | None]] = []
    ...
    async with app.run_test() as pilot:
        ...
        assert alert_widget._role_style.border == PI_DARK_THEME.error
        assert alert_widget._role_style.body == f"bold {PI_DARK_THEME.error}"
    ...
    diagnostics = (
        ResourceDiagnostic(
            ...
            message=(
                ...
                f"{tmp_path / 'home' / '.pi-agent' / 'skills' / 'review' / 'SKILL.md'}"
            ),
        ),
        ResourceDiagnostic(
            kind="prompt",
            name="ship",
            path=tmp_path / "project" / ".pi-agent" / "prompts" / "ship.md",
            message=(
                "overrides lower-precedence resource at "
...
async def test_tui_app_runs_initial_prompt() -> None:
    ...
    app = PiTuiApp(session, initial_prompt="explain this repo")
...
async def test_run_tui_app_falls_back_to_first_credentialed_provider(
    ...
    monkeypatch.setattr(tui_app, "PiTuiApp", FakeApp)
...
async def test_run_tui_app_exits_when_startup_trust_is_cancelled(
    ...
    monkeypatch.setattr(tui_app, "CodingSession", FakeCodingSession)
    monkeypatch.setattr(tui_app, "PiTuiApp", UnexpectedApp)
    ...
    class LoadedSession:
        resource_diagnostics = (
            ResourceDiagnostic(
                ...
                message=(
                    "overrides lower-precedence resource at "
                    f"{tmp_path / '.pi-agent' / 'skills' / 'review' / 'SKILL.md'}"
                ),
            ),
        )
...
async def test_run_tui_app_opens_when_provider_login_is_missing(
    ...
    class FakeApp:
        def __init__(self, session: str, **kwargs: object) -> None:
            ...
            assert "Pi 0.2.0 is available" not in message
            ...
            assert "Pi 0.2.0 is available" in notices
    ...
    await tui_app.run_tui_app(
        ...
        model=None,
        session_manager=FakeManager(),
        startup_notice="Pi 0.2.0 is available",
    )
...
# These drive the generic widget-hosting seam on the real PiTuiApp via the
...
def _component_bridge(app: PiTuiApp) -> _TuiExtensionUiBridge:
...
async def test_component_key_interceptor_preempts_priority_binding() -> None:
    ...
    PiTuiApp.on_event, ahead of that priority check, so it can own those keys.
    """
    app = PiTuiApp(FakeSession())
