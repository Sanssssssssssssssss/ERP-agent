...
- 134 次实际 MCP 分发、0 次原生分发、155 次已完成 JSON-2 尝试；16 个业务错误结果，0 个协议错误。失败完整保留，其中 4 次未先 validate 的执行被安全拒绝，模型补校验后继续；不能把这些拒绝归因为运行时间长导致批准过期。
