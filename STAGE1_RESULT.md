...
A 于 2026-09-03 15:50:34 UTC 自然结束，Agent 阶段耗时 **85 分 28.62 秒**；15:51:33 UTC trial 完成。ERP-Bench **100 分，62 / 62 条适用规则通过**（另 1 条不适用），无 Harbor 异常。不是 60 轮截断，也不是仅凭模型自称完成。

- 56 次 HTTP 请求、56 条 HTTP 200 响应头和 56 条完整模型响应；无未配对请求、无未知尾部用量，本轮未出现 `400 Content Exists Risk`。
- 新输入 **63,518**、缓存输入 **2,812,672**、输出 **311,894**，合计 **3,188,084** token；推理 **294,139** 已包含在输出内，不另加。供应商费用字段未知。
- 134 次实际 MCP 分发、0 次原生分发、155 次已完成 JSON-2 尝试；17 个业务错误结果，0 个协议错误。失败完整保留，其中 4 次未先 validate 的执行被安全拒绝，模型补校验后继续；不能把这些拒绝归因为运行时间长导致批准过期。
- 原始结果与评分位于 `.runtime/jobs/stage1-snapshot-ab-20260903-r3/2262_easy_26_buy_only_net_30_no__JJRVSob/`，逐次调用与 token 报告在 `.runtime/reports/index.html`。

B trial `2262_easy_26_buy_only_net_30_no__QcKjyNw` 已从独立实例恢复同一个快照并开始模型请求，实际原生读取分发已发生。两组的工具契约 SHA-256 均为 `7703e4c422d38e2a642da6000a1e5ee1670031a713282e66fc11a7e2cfa7a716`，系统提示词均为 `79f0b7446c1463ba669df15941857edabc2a26909223803cb58e7a39ddb2313e`；首条完整请求文件 SHA-256 也相同，为 `8cc049d2095ed99d0994cedcae2d617cdc37b3b6a5175be48d30b91312f84cb4`。未把 A 轨迹或评分答案交给 B。被测 Agent/MCP、启动与工具适配、bench/configs 保持启动版本；后续仅修离线报告。

独立子 agent 在 A 结束后发现报告少计了 1 个错误：对工具结果 JSON 文本直接做正则脱敏会吞掉引号，导致统计跳过不可解析的结果。已在共用脱敏函数中先解析 JSON、递归脱敏再序列化，补充结构化凭据字段脱敏及 1 条回归检查；完整离线检查为 **931 MCP、21 集成通过，1 历史 Node 检查跳过**（`check-20260903T160951Z`）。A 重算由 16 改为 17，分数及 token 不变，原始会话 SHA-256 仍为 `ad4a94187f15d01a050000969f551ae3417c46260280cdd8850efe85def30713`。独立复核确认导出 HTML 中 134 条工具 JSON 均可解析、真假结果与原始一致，并核对了 69 处敏感字段及合成嵌套用例。仅修派生报告，不修改模型接收的结果或任何原始回执；job 结束后还需用新版报告器重新导出，避免长驻启动进程中的旧版报告覆盖修正。

**第一阶段尚未晋升。** A 单组通过不等于 A/B 通过；仍等待 B 自然结束、全部适用规则通过及独立子 agent 最终复查，之后才能开始第二阶段。此时尚不比较两组性能或分析具体解题轨迹。
