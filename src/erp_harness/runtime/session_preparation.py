...
class PreparedCodingSession:
    ...
    async def adopt(self) -> HarnessSession:
...
async def prepare_coding_session(
    ...
    *,
    session_loader: type[HarnessSession] | None = None,
    ...
    # controlled independently by SessionConfig.owns_initial_provider.
    staged_config = replace(config, defer_authoritative_writes=True)
    loader = session_loader or HarnessSession
