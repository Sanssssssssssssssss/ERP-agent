"""Token-usage analytics for Pi sessions, rendered as an HTML dashboard."""
...
from erp_harness.providers.catalog import builtin_provider_entry, model_cost_for_input_tokens
from erp_harness.runtime.session_stats import _response_cost
from erp_harness.runtime.exporting.themes import PI_DARK_THEME, PI_LIGHT_THEME
...
_SERIES_COLORS = {
    ...
    "cached": (PI_DARK_THEME.accent, PI_LIGHT_THEME.accent),
    "cache writes": (PI_DARK_THEME.success, PI_LIGHT_THEME.success),
    "fresh": (PI_DARK_THEME.error, PI_LIGHT_THEME.error),
    "request": (PI_DARK_THEME.error, PI_LIGHT_THEME.error),
    "cumulative": (PI_DARK_THEME.markdown_link, PI_LIGHT_THEME.markdown_link),
    "output": (
        PI_DARK_THEME.role_styles["branch_summary"].border,
        PI_LIGHT_THEME.role_styles["branch_summary"].border,
    ),
    "reasoning": (PI_DARK_THEME.success, PI_LIGHT_THEME.success),
    "event": (PI_DARK_THEME.markdown_bullet, PI_LIGHT_THEME.markdown_bullet),
}
...
def _series_color_pair(name: str) -> tuple[str, str]:
    return _SERIES_COLORS.get(
        name,
        (PI_DARK_THEME.markdown_link, PI_LIGHT_THEME.markdown_link),
    )
...
def render_usage_dashboard(usage: SessionUsage) -> str:
    ...
    return (
        ...
        '<p class="usage-note">Costs use Pi\'s provider catalog rates. OAuth subscription '
...
USAGE_SCRIPT = """
    (function () {
      ...
      // The Pi palette ships both variants; switch series colors with the page theme.
      function syncTheme() {
      ...
      window.addEventListener("pi-themechange", syncTheme);
      syncTheme();
