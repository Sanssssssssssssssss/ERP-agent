"""Durable shell execution settings for Pi terminal commands."""
...
from erp_harness.runtime.project_trust import TrustDefault


class ShellConfigError(ValueError):
    """Raised when Pi shell settings are invalid."""


@dataclass(frozen=True, slots=True)
class ShellSettings:
    """Shell execution settings loaded from Pi home."""
...
def shell_settings_path(paths: RuntimePaths | None = None) -> Path:
    """Return the durable shell settings path."""
    return (paths or RuntimePaths()).home / "settings.json"


def load_shell_settings(paths: RuntimePaths | None = None) -> ShellSettings:
...
def shell_settings_from_json(data: dict[str, Any]) -> ShellSettings:
    ...
    # Pi installation cannot prevent an older installation from starting.
