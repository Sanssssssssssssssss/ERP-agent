...
from erp_harness.providers.models_dev import (
    MODELS_DEV_URL,
    NVIDIA_MODELS_URL,
...
def models_store_path(paths: RuntimePaths | None = None) -> Path:
    return (paths or RuntimePaths()).models_store_path
...
def cached_models_dev_catalog_overlay(paths: RuntimePaths | None = None) -> dict[str, Any] | None:
...
async def refresh_models_dev_catalog(
    *,
    ...
    if environ.get("PI_AGENT_OFFLINE") is not None:
    ...
    try:
        headers = {"Accept": "application/json", "User-Agent": "pi-model-catalog-refresh"}
        ...
        from erp_harness.providers.catalog_loader import builtin_source_catalog
...
def _read_cache(paths: RuntimePaths | None) -> dict[str, Any] | None:
