"""Build-time models.dev catalog generation for Pi's bundled providers."""
...
from erp_harness.providers.catalog import ProviderCatalogEntry
from erp_harness.providers.thinking import THINKING_LEVELS, ThinkingLevel
...
# Pi names providers for users; models.dev names them for its own catalog.
...
def models_dev_catalog_document(
    ...
    *,
    ...
    """Generate complete model inventories and metadata for Pi providers.
    ...
    retained as explicit Pi corrections; source rows that are deprecated or do
...
def bundled_models_dev_catalog_document() -> dict[str, Any] | None:
    ...
    try:
        text = files("erp_harness.providers").joinpath(MODELS_DEV_CATALOG_RESOURCE).read_text(encoding="utf-8")
