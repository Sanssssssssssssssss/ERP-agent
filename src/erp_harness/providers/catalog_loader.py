"""Load Pi's provider catalog from packaged and user TOML files."""
...
from erp_harness.providers.thinking import ThinkingLevel, ThinkingParameter
...
class CatalogError(ValueError):
    """Raised when a Pi catalog file is invalid."""
...
def builtin_catalog_resource_text() -> str:
    ...
    return files("erp_harness.providers").joinpath("data/catalog.toml").read_text(encoding="utf-8")


@cache
def builtin_catalog() -> tuple[ProviderCatalogEntry, ...]:
    """Return Pi's built-in catalog with generated models.dev model data."""
...
def user_catalog_path(paths: RuntimePaths | None = None) -> Path:
    """Return the user-level catalog overlay path."""
    return (paths or RuntimePaths()).home / USER_CATALOG_FILENAME


def effective_catalog(paths: RuntimePaths | None = None) -> tuple[ProviderCatalogEntry, ...]:
...
def _builtin_raw_with_cached_models(paths: RuntimePaths | None) -> dict[str, Any]:
    from erp_harness.providers.models_dev_store import cached_models_dev_catalog_overlay
...
def _builtin_raw_with_generated_models() -> dict[str, Any]:
    ...
    from erp_harness.providers.models_dev import bundled_models_dev_catalog_overlay
