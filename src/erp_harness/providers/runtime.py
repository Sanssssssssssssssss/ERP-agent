"""Runtime provider construction for Pi coding sessions."""
...
from erp_harness.providers.env import AnthropicConfig, OpenAICompatibleConfig, RuntimeProviderAuth
...
from erp_harness.runtime.hooks.providers import (
    CredentialReader,
    DynamicProvider,
...
from erp_harness.providers.oauth import (
...
from erp_harness.providers.oauth_types import OAuthProvider
...
from erp_harness.providers.thinking import ThinkingLevel, normalize_thinking_level, reasoning_effort_for_level


class ClosableModelProvider(ModelProvider, Protocol):
    """Runtime provider object Pi owns and can close."""
...
async def _resolve_dynamic_runtime_auth(
    ...
    *,
    ...
    try:
        ...
        # Preserve only Pi's exact strategy and host-authored missing-key error.
...
def create_model_provider(
    ...
    *,
    ...
    if isinstance(provider, AnthropicProviderConfig):
        ...
        config = replace(config, provider_hooks=provider_hooks)
    ...
    if isinstance(provider, OpenAICompatibleProviderConfig):
        ...
        compatible_config = replace(compatible_config, provider_hooks=provider_hooks)
