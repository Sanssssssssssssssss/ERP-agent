"""Thinking-mode primitives for Pi coding sessions."""
...
def normalize_thinking_level(value: str | None) -> ThinkingLevel:
    """Return a valid Pi thinking level or raise a user-facing error."""
    if value is None:
...
def reasoning_effort_for_level(level: str | None) -> ReasoningEffort:
    """Map Pi's UI thinking level to an OpenAI-compatible reasoning effort."""
...
def anthropic_thinking_budget_for_level(level: str | None) -> int | None:
    """Map Pi's UI thinking level to Anthropic extended-thinking tokens."""
    normalized = normalize_thinking_level(level)
    if normalized == "off":
        return None
    return {
