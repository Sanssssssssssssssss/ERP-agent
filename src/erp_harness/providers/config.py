...
from erp_harness.providers.catalog import (
...
def provider_settings_path(paths: RuntimePaths | None = None) -> Path:
    """Return the durable provider settings path."""
    return (paths or RuntimePaths()).home / "providers.json"
...
def save_provider_settings(settings: ProviderSettings, paths: RuntimePaths | None = None) -> Path:
...
def _save_migrated_provider_settings(settings: ProviderSettings, *, paths: RuntimePaths) -> None:
...
def _effective_provider_configs(paths: RuntimePaths | None = None) -> tuple[ProviderConfig, ...]:
