# Pi Agent for Python architecture

Pi Agent preserves Pi's separation of concerns:
...
- `pi_ai`: provider/model streaming and provider-neutral events.
- `pi_agent`: portable harness, loop, tools, messages, events, and sessions.
- `pi_coding`: CLI application, resources, skills, extensions, commands, persistence, rendering, and TUI integration.
...
runtimes reuse
`pi_ai.OpenAICompatibleProvider` through an explicit transport
choice, so model names cannot silently select another endpoint API.

The local-backend contract is another `pi_coding` boundary. A staged runtime
...
In a project checkout, read `AGENTS.md`, this document, and relevant
`dev-notes/architecture/` records before broad architectural changes.
