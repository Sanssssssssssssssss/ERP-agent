...
Pi resolves trust for the canonical destination cwd before reading ambient
...
The bundled llama.cpp backend is trusted Pi package code. It loads before the
...
does not scan processes, ports, or the local network. Pi does not install,
...
API keys are collected through secret fields. A stored key is kept in Pi's
private credential store and referenced by an opaque integration-owned name.
`LLAMA_API_KEY` is an environment fallback. If neither is present, Pi omits
