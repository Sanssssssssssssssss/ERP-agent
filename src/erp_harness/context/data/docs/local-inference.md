...
Pi ships llama.cpp as a trusted, hidden built-in local backend. It is exposed
...
Pi recommends router mode for `/local` model download/load/unload management:
...
There is no universally optimal llama.cpp command. For one interactive Pi user
...
The default endpoint is `http://127.0.0.1:8080`. Pi does not install, start,
stop, or scan for llama.cpp. In compatible router mode it can explicitly ask the
independent server to download a model; Pi never writes or deletes model files.
...
is the only backend. Pi immediately probes the saved endpoint,
`LLAMA_BASE_URL`, or the default `http://127.0.0.1:8080`. Use **Configure** to
enter another server URL, with or without `/v1`, and an optional API key. Pi
...
makes discovered models available for the current Pi process but does not save
...
- enter it in the secret `/local` field; Pi stores it in `~/.pi-agent/credentials.json`;
- set `LLAMA_API_KEY` for an environment-only setup;
- leave both empty for an unauthenticated server.

A stored key takes precedence over `LLAMA_API_KEY`. Without a key Pi sends no
...
```bash
pi-agent --provider llama.cpp --model <model-id>
pi-agent --provider llama.cpp --model <model-id> --print "summarize this project"
```
...
Refresh enables management only when `/props` identifies a router in Pi's
...
select a repository/quantization result. Pi shows a separate confirmation with
...
is a UI recommendation only. Pi
...
If the server no longer reports the active model, Pi keeps the current runtime
...
Reset removes only Pi's llama.cpp integration settings and safe snapshots. It
...
- **Connection refused or timeout:** start `llama-server`, check the exact URL,
  and run `/local` → Refresh. Pi does not discover a different port.
...
- **`model limit reached` during download:** the shared router rejected the
  ...
  later. Pi refreshes state and shows the router's exact rejection message.
- **Malformed or empty `/v1/models`:** inspect the server response and model
  loading state. Pi does not invent an ID or metadata.
...
provider path and are not registered as Pi backends.

Pi never copies old fake keys, fake model IDs, catalog definitions, project
