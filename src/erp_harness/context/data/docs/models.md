...
Pi includes a trusted, hidden `llama.cpp` provider layer for local inference.
...
```bash
pi-agent --provider llama.cpp --model <server-reported-id>
pi-agent --provider llama.cpp --model <server-reported-id> --print "summarize this project"
```

The built-in provider is dynamic and process-local. Its safe endpoint/model
snapshot is stored under `~/.pi-agent/state/extensions/llama.cpp.json`; its optional
secret is stored separately in `~/.pi-agent/credentials.json`. Dynamic definitions
...
Ollama adapter; Ollama is not a shipped Pi backend.
...
`pi-agent setup`, or a user-level `~/.pi-agent/catalog.toml` entry. That path is useful for
Ollama, gateways, and older manually configured local providers. An existing
provider named `llama-cpp` remains distinct from the built-in `llama.cpp`; Pi
does not migrate or overwrite it.

## Metadata and selection rules

Use exact provider/model IDs. Pi does not infer context windows, output limits,
...
model, Pi keeps the current runtime usable and marks the model snapshot stale;
...
Pi follows Pi's build-time model-generation design. Provider transports,
authentication, defaults, compatibility corrections, and fallback model rows
live in `src/pi_coding/data/catalog.toml`. Complete model inventories and
...
Pi thinking levels match Pi: `off`, `minimal`, `low`, `medium`, `high`,
...
Pi also refreshes catalogs like Pi. Opening `/model` shows the current snapshot
immediately and refreshes in the background. `pi-agent update --models` forces a
refresh. Results are ETag-revalidated, throttled to four hours, and cached at
`~/.pi-agent/models-store.json`; a cache applies only when newer than the bundled
snapshot. Since Pi has no hosted catalog service, it fetches models.dev and
NVIDIA directly and transforms them locally. `PI_AGENT_OFFLINE=1` disables catalog
network access.

Startup never requires network. Missing, invalid, or incompatible generated or
cached data falls back silently to `catalog.toml`. User `~/.pi-agent/catalog.toml`
...
Update this document, both repository READMEs, and a development note for
substantial changes. Run focused provider tests, full pytest, Ruff, formatting,
and mypy.
