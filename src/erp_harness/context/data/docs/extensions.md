...
Pi extensions are Python modules that can register custom tools, slash commands,
...
For complete API documentation, read this reference and the relevant implementation:

- `src/pi_coding/extensions/`
...
- `~/.pi-agent/extensions/`: discovered by default.
- `<project>/.pi-agent/extensions/`: requires project approval and `--project-extensions`.
- `pi-agent -e PATH`: explicitly load a file or directory.

## System-prompt contributions

Use `pi.add_prompt_guideline(text)` for one behavioral bullet in Pi's
`Guidelines` section. Use `pi.add_prompt_section(title, body)` for structured,
always-on context containing paragraphs, lists, or code blocks. `title` may be
`None`; otherwise Pi renders it as a level-two Markdown heading.
...
```bash
pi-agent install git:github.com/owner/repository
pi-agent install git:github.com/owner/repository@v1.2.0
pi-agent install ./path/to/extension.py
pi-agent install ./path/to/extension-directory
```

Git repositories and local directories install under `~/.pi-agent/extensions/` and
must contain `extension.py` or a `[tool.pi-agent].extensions` manifest. Use `--force`
...
Pi can declare product capabilities as `BuiltInExtension` values in
`pi_coding.built_in_extensions`. These call the same synchronous `setup(pi)`
...
Built-ins are trusted because their code ships inside the installed Pi package;
...
An extension defines `setup(pi)`. Built-in, user, and explicit extensions may
...
`pi.register_provider(DynamicProvider(...))` adds a frontend-free provider layer
...
Pi's actionable missing-credential guidance.
...
network policies run separately. Cancellation is generation-owned: Pi issues one
...
def setup(pi):
    ...
    pi.register_local_backend(backend)
```
...
2. In a project checkout, also read the relevant public extension API implementation.
3. Confirm the requested capability exists in the extension API before inventing a workaround.
4. Define `setup(pi)` and use documented registration APIs; do not reach into private session or Textual internals. For sidebar status, feature-detect `context.ui.sidebar`, check `sidebar.supported`, and call `set_section(...)` from `session_start` or another runtime event.
5. Keep extension behavior out of `pi_agent`; extensions belong to `pi_coding`. Use `pi_agent` types for portable messages and tools, and keep Textual behind Pi's UI adapter APIs.
6. Put user extensions in `~/.pi-agent/extensions/`. Project extensions require explicit trust through `--project-extensions`; never enable one from an untrusted repository. Use `pi-agent -e PATH` for isolated testing.
7. Test through the real extension runtime so discovery, imports, and `setup` registration are exercised. For Pi core changes, add deterministic tests with fake providers/tools and cover reload and lifecycle behavior when applicable.
8. Run focused tests followed by the repository's full pytest, Ruff, formatting, and mypy checks.
9. Update this document, both repository READMEs, and a development note for user-facing architectural changes.
