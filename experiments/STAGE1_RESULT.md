...
## 最新状态：第一阶段已验收，r3 A/B 均通过

两组均自然结束，**100 分、62 / 62 条适用规则通过，另 1 条不适用；Harbor 异常为 0**。整对 job 于 2026-09-03 16:17:29 UTC 完成，宿主包装进程正常退出。只验证四个原生读取工具的功能链路，不代表已完成全部 Odoo 原生化。

| 指标 | A：全部 MCP | B：四个原生读取，其余 MCP |
| --- | ---: | ---: |
| 模型请求 / 完整响应 / HTTP 200 | 56 / 56 / 56 | 72 / 72 / 72 |
| 工具调用 | 134 | 163 |
| 实际 MCP 分发 | 134 | 75 |
| 实际原生分发 | 0 | 88 |
| 已完成 JSON-2 尝试 | 155 | 144 |
| 业务错误结果 / 协议错误 | 17 / 0 | 27 / 0 |
| 新输入 token | 63,518 | 56,318 |
| 缓存输入 token | 2,812,672 | 3,258,112 |
| 输出 token | 311,894 | 135,589 |
| 其中推理 token，已含在输出内 | 294,139 | 113,276 |
| 总 token | 3,188,084 | 3,450,019 |
| Agent 阶段墙钟时间 | 85 分 28.62 秒 | 24 分 27.26 秒 |

两组均无未配对请求、未完成工具调用或未知尾部用量，未出现 `400 Content Exists Risk`；费用金额未知，不能记作零。**这是一对功能验证，不是性能提升证明。** 模型轨迹不同；B 虽然耗时较少，但调用数、总 token 和业务错误更多，不能归因为换了原生传输。

B 的 88 次原生调用分别为：`get_odoo_profile` 1、`get_model_fields` 16、`search_records` 53、`read_record` 18；其余 75 次仍走 MCP。B 完成了 72 次模型响应，证明确实没有被旧 60 轮保护截断。
...
r3 已从推送检查点 `5f61f3804b4cf985e0ad24f7fd8422bcda4f3179` 启动，配置 SHA-256 为 `17fcbe272eca1bdecf2acff3a6c82534bbaae2cb35d0a14a947e58e8b1cccd8c`。发车前离线检查仍为 931 MCP、20 集成通过，1 历史 C Node 检查跳过（`check-20260903T142221Z`）；Harbor 配置校验和独立子 agent 确认没有隐藏的 1,800/3,600 秒截断。A trial `2262_easy_26_buy_only_net_30_no__JJRVSob` 恢复并验证相同快照后才开始模型调用。来源回执在启动前保存于 `.runtime/stage1/r3-experiment.json`，Harbor 建立 job 后复制到该 job 的 `experiment.json`。运行期间不修改被测 Agent/MCP 代码。

### 条件、回执与报告修正

A 的原始结果与评分位于 `.runtime/jobs/stage1-snapshot-ab-20260903-r3/2262_easy_26_buy_only_net_30_no__JJRVSob/`。错误记录完整保留，其中 4 次未先 validate 的执行被安全拒绝，模型补校验后继续；不能把这些拒绝归因为运行时间长导致批准过期。两组 trial 完成后，Harbor 已清理各自的可丢弃容器，原始回执不删除。

B trial `2262_easy_26_buy_only_net_30_no__QcKjyNw` 从独立实例恢复同一个快照后运行，现已完成；其原始会话 SHA-256 为 `8a27ee2e4f7aec90be122d6e394387f8cfe00a35c963263e958bbd55430fe221`。两组的工具契约 SHA-256 均为 `7703e4c422d38e2a642da6000a1e5ee1670031a713282e66fc11a7e2cfa7a716`，系统提示词均为 `79f0b7446c1463ba669df15941857edabc2a26909223803cb58e7a39ddb2313e`；首条完整请求文件 SHA-256 也相同，为 `8cc049d2095ed99d0994cedcae2d617cdc37b3b6a5175be48d30b91312f84cb4`。未把 A 轨迹或评分答案交给 B。被测 Agent/MCP、启动与工具适配、bench/configs 保持启动版本；后续仅修离线报告。

独立子 agent 在 A 结束后发现报告少计了 1 个错误：对工具结果 JSON 文本直接做正则脱敏会吞掉引号，导致统计跳过不可解析的结果。修复检查点为 `e44d6858fe31e2bd7ee66c3d2112733d0e9a5ea0`：在共用脱敏函数中先解析 JSON、递归脱敏再序列化，补充结构化凭据字段脱敏及 1 条回归检查；完整离线检查为 **931 MCP、21 集成通过，1 历史 Node 检查跳过**（`check-20260903T160951Z`）。A 重算由 16 改为 17，分数及 token 不变，原始会话 SHA-256 仍为 `ad4a94187f15d01a050000969f551ae3417c46260280cdd8850efe85def30713`。独立复核确认导出 HTML 中 134 条工具 JSON 均可解析、真假结果与原始一致，并核对了 69 处敏感字段及合成嵌套用例。仅修派生报告，不修改模型接收的结果或任何原始回执；已在整个 job 和旧包装进程退出后，用新进程重新导出，最终报告的错误数为 A 17、B 27。

**第一阶段已通过独立子 agent 最终验收。** 原始评分、自然停止、分发配对、128 条实际请求无输出上限、用量及两份会话指纹均一致。B 的 14 个原生失败结果经固定输入与固定 Odoo 错误的零网络回放，MCP/原生的传参及错误 envelope 均相同：13 个无效字段/条件，1 个 `record_id=0`；其余 13 个失败走的是既有 MCP。没有发现 B 原生实现回归证据。两组最终 HTML 的工具 JSON 全部可解析，错误数与原始 17 / 27 一致。

此检查点可以称为“四个原生读取工具的已验收混合运行版本”，不能称为完整无 MCP Harness。保存 Git 检查点后进入第二阶段；只做 A/B，不读或实现 Compiler。原始 job 位于 `.runtime/jobs/stage1-snapshot-ab-20260903-r3/`，逐次调用及用量位于 `.runtime/reports/index.html`，不上传带业务内容的原始回执或本地密钥。

## 历史 r2：A 主动中断，B 未启动；修复超时生命周期
