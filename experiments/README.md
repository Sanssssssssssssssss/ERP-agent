...
更新：2026-09-04。初始路线图依据本仓库代码检查点 `8a83f5c` 和用户提供的七阶段方案编写；第一阶段实际验收版本及证据见结果文档。

本文件是**实现路线图，不是完成报告**。用户已授权按阶段执行长任务：每阶段通过离线检查、真实 Odoo 检查、有界模型实验和独立子 agent 复查后，才能进入下一阶段。第一至第四阶段均已验收；第五阶段的 5.1 原生能力与 5.2 受控 SOP 已验收，5.3 动态工具已实现但因真实 A/B 质量回退而暂停验收；第六阶段原生知识和第七阶段无 MCP 原生部署均已验收。证据和历史失败见 [STAGE1_RESULT.md](STAGE1_RESULT.md)、[STAGE2_RESULT.md](STAGE2_RESULT.md)、[STAGE3_RESULT.md](STAGE3_RESULT.md)、[STAGE4_RESULT.md](STAGE4_RESULT.md)、[STAGE5_RESULT.md](STAGE5_RESULT.md)、[STAGE6_RESULT.md](STAGE6_RESULT.md) 与 [STAGE7_RESULT.md](STAGE7_RESULT.md)。

**最新执行授权（2026-09-03）：** 用户允许重跑，并明确“时间和预算先不管，我们先看功能”。因此下文的初始“一对、每组 1,800 秒、额外重跑须申请预算”不再作为本轮功能验证的停止条件。先跑同快照的 case 2262 A/B，普通缺陷继续修复验证，不启动无关大批量任务。当前两组采用相同的 86,370 秒运行截止、86,400 秒 Harbor 外层兜底，作为 24 小时遗留进程保护，不是无限运行；轮数和输出 token 不设上限。异常/取消清理、单请求网络超时、权限/副作用边界及逐阶段验收保持不变。仍不做 C 或 Compiler。
...
第一阶段 A0 还包含两组相同的修正：Agent/MCP 安装到容器内独立 venv，不修改 Odoo 系统 Python；`health_check` 的进程局部 N+1 计数和热点请求计数只保留在原始回执，不投影到模型上下文。权限、策略和限速配置仍保留。原因是 B 的原生请求不经过 MCP 进程，旧计数不能代表整个运行时；这不是只对 B 隐藏不利结果。第二阶段再实现统一原生请求观测。当前 venv 仍继承镜像系统包，完全封闭安装留到第七阶段验收。

共同保持：模型及 provider、reasoning、输入任务、初始数据、Odoo 身份及公司上下文、公共安全策略和外层时间预算。当前两组共同使用 86,370 秒运行截止、86,400 秒 Harbor 外层兜底，不恢复客户端输出 token 上限。若以后需要额外费用或 API 次数保护，必须在实验清单中事先声明并同样作用于两组，不能暗中加回 60 轮。

第一轮 A 运行中的独立复查发现 Harbor 外层取消不能保证停止容器内进程，因此共同 A0 补上真实截止：运行阶段 1,770 秒（扣除启动耗时），完整模型管道由 GNU `timeout` 管理，TERM 后 5 秒仍未退出则 KILL；失败或取消时等待 `stop_service("main")` 完成。外层 1,800 秒通常留下 30 秒清理余量，但取消仍须等待容器停止，不能承诺整项 job 的硬性墙钟上限。超时轮保留异常、不继续 shared 评分；正常结束才进入 ERP 评分。该变化两组相同，原始中断 A 保留、不并入有效配对。
...
**已验收阶段。** 11 个读取工具均有原生去向；确定性检查为 931 项 MCP 与 24 项 Harness 通过（1 项既有 Node-only 跳过），真实 Odoo 双身份 gate 为 42/42、0 模型调用。case 2262 的一对 A/B 均自然结束、100 分、62/62 适用规则通过；独立复核无阻断。详细回执、限制和证据卫生项见 [STAGE2_RESULT.md](STAGE2_RESULT.md)。写入及其他未迁能力仍走 MCP，未运行 C 或 Compiler。
...
**已验收阶段。** World record core 已实现身份隔离的观测、局部字段合并、关系、错误、版本、失效、恢复和闭合报告。确定性检查为 33 项 Harness 通过（1 项既有 Node-only 跳过）与 931 项 MCP 通过；真实 Odoo 双身份 gate 为 42/42、0 模型调用且验证外部更新。case 2262 的 A=`record`、B=`project` 均自然结束、100 分、62/62 适用规则通过，合计 114 个模型请求全部 HTTP 200。B 的 65 次投影调用没有替换任何消息，因此不宣称 token 收益；只晋升 record core，project 继续 opt-in，默认 off。详细证据与 r1 失败分析见 [STAGE3_RESULT.md](STAGE3_RESULT.md)。未运行 C 或 Compiler。
...
**已验收阶段。** 原生动作统一门控、SQLite 持久账本、发送后未知结果对账、执行后验证和报告重算均已实现。确定性 Harness 为 55 项通过（含 29 个 subtests），真实 Odoo 动作 gate 为 11/11、0 模型调用；同提交 case 2262 的固定 A 与补跑 B 均自然结束、100 分、62/62 适用规则通过。r7 的 B=22.77 规划失败完整保留，不归因于动作后端。详细证据见 [STAGE4_RESULT.md](STAGE4_RESULT.md)。
...
**已验收阶段。** 检查点 `54370d5` 的原生知识索引、权限/字段策略隔离、稳定分页和候选实时复核已通过确定性测试及无模型真实 Odoo 门禁；上一检查点的两个 A/B 案例没有显示 native 知识质量回退。r7 的 native B 因 provider 连续返回 reasoning-only 空响应成为无效样本；按用户随后指定的 B-only 规则，同代码 r8 重跑自然结束、100 分、62/62，16 个动作全部 verified，知识及后端回执完整。独立子 agent 复核后结论为 `ACCEPT`。详见 [STAGE6_RESULT.md](STAGE6_RESULT.md)。
...
**已验收阶段。** 实现检查点 `7644f1a` 已把固定工具契约和所需 Odoo 纯业务代码收进原生运行时，并把 MCP SDK 改成可选依赖。同题 B-only 真实 API 验证得到 100 分、62/62，运行收据确认三个 MCP 包不可导入、没有 MCP 进程/端口/PID，实际 `mcp_calls=0`，全部后端为 native。详见 [STAGE7_RESULT.md](STAGE7_RESULT.md)。以下保留原验收要求作为边界记录。
...
**当前状态：** 保持 5.3 动态工具暂停并继续使用 static。第七阶段已验收，当前产物为“不含 Compiler 的 Odoo 原生 Harness”；Compiler 仍未读取、移植或运行。若继续研究，应把动态工具复验、性能重复样本或 Compiler 6B 分成独立实验变量。
