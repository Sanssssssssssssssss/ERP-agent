...
0.5 已支持将 CSV 直接拖入工作台材料入口；材料只作为不可信业务参考，Agent 仍需在 Odoo 中独立读取和核对客户、商品、库存、供应商和付款条件，再按审批流程执行。两轮示例：第一轮上传后说“只处理 Nimbus 的 8 件，先整理待创建单据，缺信息只补问”；第二轮说“沿用 Odoo 默认价格、客户地址和税率，继续这个 Nimbus 单”。
