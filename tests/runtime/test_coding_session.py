...
from erp_harness.runtime.messages import AssistantMessageDiagnostic, assistant_content
from erp_harness.runtime.provider_events import AssistantErrorEvent
...
from erp_harness.providers import CancellationToken, FakeProvider, ModelProvider, RuntimeModelLimits
...
from erp_harness.providers.credentials import FileCredentialStore
from erp_harness.runtime.session import ModelChoice
from erp_harness.providers.config import OpenAICodexProviderConfig
from erp_harness.providers.config import OpenAICompatibleProviderConfig
from erp_harness.context.paths import RuntimePaths
from erp_harness.context.resources import ResourcePaths
from erp_harness.providers.config import ProviderConfigError
from erp_harness.providers.config import ProviderSettings
from erp_harness.context.resources import ResourceError
from erp_harness.providers.config import ScopedModelConfig
from erp_harness.runtime.session_manager import SessionManager
from erp_harness.runtime.session import SessionTreeBranchResult
from erp_harness.providers.config import load_provider_settings
from erp_harness.providers.config import save_provider_settings
import erp_harness.runtime.session as coding_session_module
from erp_harness.runtime.session_events import AgentSettledEvent, QueueUpdateEvent
from erp_harness.runtime.hooks import (
    BeforeAgentStartOutcome,
    DynamicProvider,
    OpenAICompatibleTransport,
    ProviderModel,
)
from erp_harness.runtime.hooks.runtime import InputHookOutcome
from erp_harness.context.prompt_templates import PromptTemplate
from erp_harness.providers.config import ProviderModelMetadata
from erp_harness.runtime.session import (
    _find_pi_compaction_cut,
...
async def test_load_empty_session_defers_transcript_file(tmp_path: Path) -> None:
    ...
    session = await HarnessSession.load(_config(tmp_path, FakeProvider([]), storage))
...
async def test_session_export_defaults_to_cwd(tmp_path: Path) -> None:
    storage = JsonlSessionStorage(tmp_path / ".pi-agent" / "sessions" / "session-1.jsonl")
    session = await HarnessSession.load(_config(tmp_path, FakeProvider([]), storage))
...
async def test_session_export_writes_jsonl_to_destination_directory(tmp_path: Path) -> None:
    ...
    assert "You are Pi." not in jsonl
...
async def test_prompt_logs_unexpected_agent_call_exception(tmp_path: Path) -> None:
    ...
    session = await HarnessSession.load(
        SessionConfig(
            ...
            resource_paths=ResourcePaths(root=pi_paths.home, paths=pi_paths),
        )
    )
    ...
    assert entry["cwd"].casefold() == str(tmp_path).casefold()
    assert entry["error"] == {"message": "provider exploded", "stop_reason": "error"}
    failure = next(
        event.message
        for event in events
...
async def test_message_persistence_retry_is_idempotent(tmp_path: Path, phase: str) -> None:
    ...
    assert sorted(entry.message.text for entry in messages) == ["", "go"]
    failure = next(entry.message for entry in messages if entry.message.text == "")
    assert isinstance(failure, AssistantMessage)
    assert failure.stop_reason == "error"
    assert len([entry for entry in messages if entry.message.text == "go"]) == 1
    restored = await HarnessSession.load(
        SessionConfig(
            provider=FakeProvider([]),
            model="fake",
            system="You are Pi.",
    ...
    assert [message.text for message in restored.messages].count("go") == 1
...
@pytest.mark.anyio
async def test_session_refreshes_provider_model_system_and_tools_between_tool_turns(
    tmp_path: Path,
) -> None:
    next_provider = FakeProvider(
        [
            [
                assistant_start(model="new-model"),
                assistant_done(AssistantMessage(content="done", model="new-model")),
            ]
        ]
    )
    session_ref: dict[str, HarnessSession] = {}
    ...
    replacement_tool = AgentTool(
        name="replacement",
        label="Replacement",
        description="Replacement",
        parameters={"type": "object"},
        execute_fn=next_tool,
    )
    ...
    async def switch_state(
        ...
        del tool_call_id, arguments, signal, on_update
        harness = session_ref["session"]._harness
    ...
    switch_tool = AgentTool(
        name="switch",
        label="Switch",
        description="Switch active state",
        parameters={"type": "object"},
    ...
    assert len(first_provider.calls) == len(next_provider.calls) == 1
...
async def test_cancelled_continue_dispatches_agent_settled_after_teardown(
    ...
    storage = JsonlSessionStorage(tmp_path / "session.jsonl")
    await storage.append(MessageEntry(message=UserMessage(content="Continue")))
...
@pytest.mark.anyio
async def test_load_preserves_orphaned_tool_result(tmp_path: Path) -> None:
    storage = JsonlSessionStorage(tmp_path / "session.jsonl")
    ...
    _assert_messages(
        session.messages,
        [
            UserMessage(content="what remains?"),
            orphan_entry.message,
...
@pytest.mark.anyio
async def test_prompt_persists_user_and_assistant_entries(tmp_path: Path) -> None:
    storage = JsonlSessionStorage(tmp_path / "session.jsonl")
    provider = FakeProvider(
        [
            [
...
async def test_terminal_command_can_persist_output_to_context(tmp_path: Path) -> None:
    ...
    session = await HarnessSession.load(_config(tmp_path, FakeProvider([]), storage))
    command = (
        "powershell -NoProfile -Command \"[Console]::Write('hello')\""
...
async def test_terminal_command_can_run_without_context(tmp_path: Path) -> None:
    ...
    session = await HarnessSession.load(_config(tmp_path, FakeProvider([]), storage))
    command = (
        "powershell -NoProfile -Command \"[Console]::Write('hidden')\""
        if sys.platform == "win32"
        else "printf hidden"
    )
...
async def test_tree_can_branch_from_first_user_message_before_assistant_response(
    ...
    with pytest.raises(RuntimeError, match="Pi is still working"):
    ...
    assert entries[-1] == message_entries[-1]
    assert session.state.active_leaf_id == message_entries[0].parent_id
...
@pytest.mark.anyio
async def test_branch_to_entry_preserves_orphaned_tool_result(tmp_path: Path) -> None:
    storage = JsonlSessionStorage(tmp_path / "session.jsonl")
    ...
    assert result.message == "Branched session at answer."
    _assert_messages(
        session.messages,
        [UserMessage(content="start"), orphan.message, AssistantMessage(content="answer")],
    )
    diagnostics = [
        entry
        for entry in (await storage.read_all())
...
async def test_context_usage_recalculates_after_prompt_and_compaction(tmp_path: Path) -> None:
    ...
    provider = FakeProvider(
        [
            [
                assistant_start(model="fake"),
                assistant_done(
                    message=AssistantMessage(content="Long answer " * 20_000),
                ),
            ],
            [
                assistant_start(model="fake"),
                assistant_done(message=AssistantMessage(content="Recent answer")),
            ],
            [
                assistant_start(model="fake"),
    ...
    recent_prompt = "Keep recent context.\n" + ("recent " * 12_000)
    _recent_events = await _collect_session_events(session.prompt(recent_prompt))
...
async def test_session_persists_and_replays_thinking_level_changes(tmp_path: Path) -> None:
    ...
    restored = await HarnessSession.load(_config(tmp_path, FakeProvider([]), storage))
...
async def test_session_persists_thinking_preference_for_new_sessions(tmp_path: Path) -> None:
    pi_paths = RuntimePaths(home=tmp_path / ".pi-agent")
    ...
    saved = load_provider_settings(pi_paths)
    assert saved.get_provider("openai-codex").thinking_defaults == {"gpt-5.5": "low"}

    new_session = await HarnessSession.load(
        SessionConfig(
            provider=FakeProvider([]),
            model="gpt-5.5",
            system="You are Pi.",
...
@pytest.mark.anyio
async def test_branch_reset_is_in_memory_until_the_next_append(tmp_path: Path) -> None:
    storage = JsonlSessionStorage(tmp_path / "session.jsonl")
    ...
    result = await session.branch_to_entry("root")
    reloaded = await HarnessSession.load(_config(tmp_path, FakeProvider([]), storage))
...
@pytest.mark.anyio
async def test_load_restores_latest_branch(tmp_path: Path) -> None:
    storage = JsonlSessionStorage(tmp_path / "session.jsonl")
    root = MessageEntry(id="root", message=UserMessage(content="Root"))
    left = MessageEntry(
        id="left",
...
async def test_session_branches_to_previous_entry_without_destroying_history(
    ...
    assert entries[-1] == right
    assert session.state.active_leaf_id == "left"
...
async def test_persist_after_branch_keeps_state_on_active_branch(tmp_path: Path) -> None:
    ...
    _events = await _collect_session_events(session.prompt(new_followup))
    ...
    _assert_messages(
        ...
        (
            ...
            UserMessage(content=new_followup),
...
@pytest.mark.anyio
async def test_session_branch_with_summary_attaches_before_selected_user_message(
    tmp_path: Path,
) -> None:
    storage = JsonlSessionStorage(tmp_path / "session.jsonl")
    first_model = ModelChangeEntry(id="model-a", model="first-model")
    ...
    await storage.append(right)
    provider = FakeProvider(
        [[assistant_start(), assistant_done(AssistantMessage(content="Abandoned model branch."))]]
    )
    session = await HarnessSession.load(_config(tmp_path, provider, storage))
    ...
    assert isinstance(session.messages[0], BranchSummaryMessage)
    assert "Abandoned model branch." in session.messages[0].summary
...
async def test_session_branch_with_summary_rebuilds_context(tmp_path: Path) -> None:
    ...
    assert summary.branch_root_id is None
    ...
    assert len(session.messages) == 1
    assert isinstance(session.messages[0], BranchSummaryMessage)
    assert "The abandoned branch went left." in session.messages[0].summary
...
@pytest.mark.anyio
async def test_session_branch_with_summary_raises_when_model_summary_is_unavailable(
    tmp_path: Path,
) -> None:
    storage = JsonlSessionStorage(tmp_path / "session.jsonl")
    root = MessageEntry(id="root", message=UserMessage(content="Root"))
    ...
    with pytest.raises(RuntimeError, match="ended without a terminal response"):
        await session.branch_to_entry("root", summarize=True)
    assert not any(entry.type == "branch_summary" for entry in await storage.read_all())
...
async def test_session_preserves_explicit_empty_system_prompt(tmp_path: Path) -> None:
    ...
    session = await HarnessSession.load(config)
...
async def test_session_builds_system_prompt_when_system_is_omitted(tmp_path: Path) -> None:
    ...
    config = SessionConfig(
        ...
        resource_paths=ResourcePaths(root=resource_root, agents_root=None),
    )
    session = await HarnessSession.load(config)
...
async def test_session_touches_session_manager_after_persisting_messages(tmp_path: Path) -> None:
    storage = JsonlSessionStorage(tmp_path / "session.jsonl")
    manager = SessionManager(RuntimePaths(home=tmp_path / ".pi-agent", agents_home=tmp_path / ".agents"))
    ...
    config = SessionConfig(
        ...
        resource_paths=ResourcePaths(root=tmp_path / "resources", agents_root=None),
    )
    session = await HarnessSession.load(config)
...
async def test_session_yields_expanded_custom_prompt_before_auto_naming(tmp_path: Path) -> None:
    ...
    session = await HarnessSession.load(
        SessionConfig(
            ...
            session_id=record.id,
            session_manager=manager,
            resource_paths=ResourcePaths(root=resources_root, agents_root=None),
        )
    )
...
async def test_session_skills_disabled_suppresses_skill_index(tmp_path: Path) -> None:
    ...
    assert session.expand_prompt_text("/skill:testing") == "/skill:testing"
...
async def test_system_command_shows_prompt_without_persisting_or_adding_context(
    ...
    assert result.message == "You are Pi."
...
async def test_session_loads_with_resource_diagnostics_instead_of_failing(
    ...
    (skills_dir / "legacy.md").write_text(
        "---\nname: legacy\ndescription: Legacy bare-md skill\n---\nBody", encoding="utf-8"
    )
    (skills_dir / "invalid.md").write_text("# Missing description", encoding="utf-8")
    storage = JsonlSessionStorage(tmp_path / "session.jsonl")
    config = SessionConfig(
        provider=FakeProvider([]),
        model="fake",
    ...
    assert {skill.name for skill in session.skills} == {"good", "legacy"}
    assert len(session.resource_diagnostics) == 1
    assert "description is required" in session.resource_diagnostics[0].message
    assert "Resource diagnostics: 1" in (session.handle_command("/session").message or "")


@pytest.mark.anyio
async def test_session_loads_pi_native_system_prompt_files(tmp_path: Path) -> None:
    pi_home = tmp_path / "pi-home"
    project_pi = tmp_path / ".pi-agent"
    pi_home.mkdir()
    project_pi.mkdir()
    (pi_home / "SYSTEM.md").write_text("User base", encoding="utf-8")
    (project_pi / "SYSTEM.md").write_text("Project base", encoding="utf-8")
    (pi_home / "APPEND_SYSTEM.md").write_text("User append", encoding="utf-8")
    (project_pi / "APPEND_SYSTEM.md").write_text("Project append", encoding="utf-8")
    ...
    assert "Current date:" not in session.system_prompt
    assert (
        f"Current working directory: {tmp_path.as_posix()}".casefold()
        in session.system_prompt.casefold()
    )
    assert session.system_prompt_files == (
        project_pi / "SYSTEM.md",
        project_pi / "APPEND_SYSTEM.md",
    )
    prompt_diagnostics = [
...
@pytest.mark.anyio
async def test_before_agent_start_system_override_is_per_run(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    provider = FakeProvider(
    ...
    async def before_agent_start(prompt: str, system_prompt: str) -> BeforeAgentStartOutcome:
        del prompt, system_prompt
        nonlocal call_count
        call_count += 1
        return BeforeAgentStartOutcome(
            system_prompt="one-run override" if call_count == 1 else None
        )

    monkeypatch.setattr(
        session._extension_runtime,  # noqa: SLF001 - parity regression at the session boundary
        "before_agent_start",
        before_agent_start,
    )
    ...
    assert provider.calls[0][1] == "one-run override"
    assert provider.calls[1][1] == session.system_prompt
...
async def test_explicit_base_overrides_file_while_explicit_append_composes(
    ...
    assert session.system_prompt_files == (project_pi / "APPEND_SYSTEM.md",)
    prompt_diagnostics = [
        item for item in session.resource_diagnostics if item.kind == "system-prompt"
    ]
    assert "explicit startup value" in prompt_diagnostics[0].message
    assert "selected project" in prompt_diagnostics[1].message
...
async def test_session_reload_tracks_system_prompt_file_precedence(tmp_path: Path) -> None:
    ...
    pi_home.mkdir()
    project_pi.mkdir()
    user_prompt = pi_home / "SYSTEM.md"
    project_prompt = project_pi / "SYSTEM.md"
    append_prompt = pi_home / "APPEND_SYSTEM.md"
    session = await HarnessSession.load(
        SessionConfig(
            provider=FakeProvider([]),
            model="fake",
...
async def test_new_session_adopts_system_prompt_resource_tracking(tmp_path: Path) -> None:
    ...
    manager = SessionManager(RuntimePaths(home=pi_home, agents_home=tmp_path / "agents-home"))
    record = manager.create_session(cwd=tmp_path, model="fake")
    session = await HarnessSession.load(
        SessionConfig(
            provider=FakeProvider([]),
...
async def test_session_reload_skips_provider_settings_refresh(
    ...
    def fail_load_provider_settings(paths: RuntimePaths | None = None) -> ProviderSettings:
        del paths
...
async def test_session_provider_settings_reload_uses_session_paths(
    ...
    pi_paths = RuntimePaths(home=tmp_path / "pi-home", agents_home=tmp_path / "agents-home")
    seen_paths: list[RuntimePaths | None] = []

    def load_provider_settings(paths: RuntimePaths | None = None) -> ProviderSettings:
...
async def test_session_compact_persists_summary_and_rebuilds_context(tmp_path: Path) -> None:
    ...
    provider = FakeProvider(
        [
            [
                assistant_start(model="fake"),
                assistant_done(message=AssistantMessage(content="Session answer " * 20_000)),
            ],
            [
                assistant_start(model="fake"),
                assistant_done(message=AssistantMessage(content="Recent answer")),
    ...
    assert compactions[0].replaces_entry_ids == message_entries_before[:2]
    assert compactions[0].first_kept_entry_id == message_entries_before[2]
    assert entries_after_compact[-1] == compactions[0]
    assert provider.calls[2][1].startswith("You are a context summarization assistant.")
    assert "Additional focus: Focus on session persistence." in provider.calls[2][2][0].content
    assert [message.role for message in provider.calls[3][2]] == [
        "user",
        "user",
        "assistant",
        "user",
    ]
    assert "Generated session summary" in provider.calls[3][2][0].text
    assert provider.calls[3][2][1].text == recent_prompt
    assert provider.calls[3][2][2].text == "Recent answer"
    assert provider.calls[3][2][3].text == "Continue."


def test_compaction_cut_keeps_default_when_large_tool_result_has_no_later_cut() -> None:
    rows = (
        ("user", UserMessage(content="Run the tool")),
        ("assistant", AssistantMessage(content="Calling it")),
        (
            "tool",
            ToolResultMessage(
                tool_call_id="call-1",
    ...
    assert _find_pi_compaction_cut(rows, start_index=0, keep_recent_tokens=1) == (0, None)
...
async def test_session_auto_compacts_after_response_when_threshold_is_exceeded(
    ...
    recent_prompt = "Continue.\n" + ("recent " * 12_000)
    _second_events = await _collect_session_events(session.prompt(recent_prompt))
    ...
    assert compactions[0].summary in provider.calls[3][2][0].text
    assert provider.calls[3][2][1].text == recent_prompt
    assert provider.calls[3][2][2].text == "Second answer"
    assert provider.calls[3][2][3].text == "Next."
...
async def test_session_auto_compacts_with_pi_style_default_threshold(
    ...
    _first_events = await _collect_session_events(session.prompt(large_prompt))
    _second_events = await _collect_session_events(
...
async def test_session_compacts_and_retries_once_after_context_overflow(
    ...
    extension_events = _record_extension_events(session, monkeypatch)
    _first_events = await _collect_session_events(session.prompt(large_prompt))
    recent_prompt = "Keep this recent turn.\n" + ("recent " * 12_000)
    _second_events = await _collect_session_events(session.prompt(recent_prompt))
    ...
    assert "Overflow recovery summary" in provider.calls[4][2][0].text
    assert [message.text for message in provider.calls[4][2][1:4]] == [
        recent_prompt,
        "Second answer",
        "Trigger overflow.",
    ]
...
@pytest.mark.anyio
async def test_disabled_auto_compaction_does_not_retry_context_overflow(tmp_path: Path) -> None:
    storage = JsonlSessionStorage(tmp_path / "session.jsonl")
    provider = FakeProvider(
        [[assistant_error(message="This model's maximum context length was exceeded.")]]
    )
    ...
    events = await _collect_session_events(session.prompt("Do not compact."))

    assert len(provider.calls) == 1
    assert not any(getattr(event, "type", None) == "compaction_start" for event in events)
    assert not any(entry.type == "compaction" for entry in await storage.read_all())


@pytest.mark.anyio
async def test_failed_overflow_compaction_does_not_retry(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    provider = FakeProvider(
    ...
    def fail_plan() -> None:
        raise RuntimeError("compaction plan failed")

    monkeypatch.setattr(session, "_recent_preserving_compaction_plan", fail_plan)
    events = await _collect_session_events(session.prompt("Trigger overflow."))

    [compaction_end] = [
        event for event in events if getattr(event, "type", None) == "compaction_end"
    ]
    assert len(provider.calls) == 1
...
@pytest.mark.anyio
async def test_session_compacts_and_retries_once_after_recoverable_length(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    provider = FakeProvider(
        [
            [assistant_start(), assistant_done(truncated, "length")],
            [assistant_start(), assistant_done(AssistantMessage(content="Recovered answer"))],
        ]
    )
    provider_config = OpenAICompatibleProviderConfig(
        name="openai",
        models=("fake",),
        default_model="fake",
        model_metadata={"fake": ProviderModelMetadata(max_tokens=1_000)},
    )
    session = await HarnessSession.load(
        SessionConfig(
            provider=provider,
    ...
    async def compact(*, context: object) -> bool:
        del context
        return True

    monkeypatch.setattr(session, "_try_overflow_compact", compact)
    events = await _collect_session_events(session.prompt("Continue a long task."))

    assert len(provider.calls) == 2
    assert any(
        isinstance(event, MessageEndEvent)
        and isinstance(event.message, AssistantMessage)
        and event.message.text == "Recovered answer"
        for event in events
    )
    assert [getattr(event, "type", None) for event in events].count("compaction_start") == 1
...
async def test_huggingface_automatic_pin_fails_over_and_repins(
    ...
    pi_paths = RuntimePaths(home=tmp_path / ".pi-agent", agents_home=tmp_path / ".agents")
    manager = SessionManager(pi_paths)
    record = manager.create_session(
        cwd=tmp_path,
        model=model,
        provider_name="huggingface",
    ...
    diagnostic = json.loads(pi_paths.agent_calls_log_path.read_text().splitlines()[-1])
    ...
    session = await HarnessSession.load(
        SessionConfig(
            ...
            resource_paths=ResourcePaths(
                ...
                paths=RuntimePaths(home=tmp_path / ".pi-agent", agents_home=tmp_path / ".agents"),
            ),
        )
    )
...
async def test_huggingface_fixed_pin_does_not_fail_over(
    ...
    assert session.inference_provider_mode == "fixed"
    [retry_start] = [
        event for event in events if getattr(event, "type", None) == "auto_retry_start"
    ]
    assert retry_start.error_message == "Rate limit exceeded"
    agent_ends = [event for event in events if getattr(event, "type", None) == "agent_end"]
    assert [event.will_retry for event in agent_ends] == [True, False]
...
async def test_session_adoption_transfers_all_runtime_provider_ownership(
    ...
    if operation == "new":
        ...
        replacement = await HarnessSession.load(
            dataclass_replace(
...
async def test_session_switch_uses_session_credential_store(
    ...
    pi_paths = RuntimePaths(home=tmp_path / "pi-home", agents_home=tmp_path / "agents-home")
    FileCredentialStore(pi_paths.home / "credentials.json").set("openai", "stored-key")
    ...
    assert credential_store_paths == [pi_paths.home / "credentials.json"]
...
async def test_session_toggles_and_cycles_scoped_models(
    ...
    saved = json.loads((pi_paths.home / "providers.json").read_text(encoding="utf-8"))
...
async def test_session_toggle_scoped_model_preserves_newer_provider_file_changes(
    ...
    save_provider_settings(newer_settings, pi_paths)
    session = await HarnessSession.load(
        SessionConfig(
            provider=FakeProvider([]),
            model="qwen",
    ...
    saved = coding_session_module.load_provider_settings(pi_paths)
...
async def test_session_set_model_preserves_newer_provider_file_changes(
    ...
    saved = coding_session_module.load_provider_settings(pi_paths)
    assert saved.get_provider("openai").default_model == "gpt-5-mini"
    assert saved.get_provider("openrouter").headers == {"X-Title": "Pi"}
    assert saved.scoped_models == (
...
async def test_resumed_session_thinking_override_is_ephemeral(
    ...
    def config(thinking_level_override: object = None) -> SessionConfig:
        return SessionConfig(
            provider=FakeProvider([]),
            model="reasoner",
            system="You are Pi.",
    ...
    with pytest.raises(ProviderConfigError, match='Thinking mode "medium" is not available'):
        await HarnessSession.load(config(thinking_level_override="medium"))
...
def _dynamic_thinking_override_config(
    ...
    *,
    ...
    extension.write_text(
        """
from erp_harness.runtime.hooks import DynamicProvider, OpenAICompatibleTransport, ProviderModel


def setup(pi):
    pi.register_provider(DynamicProvider(
        id="local",
        display_name="Local",
