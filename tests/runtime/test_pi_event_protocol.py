...
def test_pi_agent_does_not_import_pi_ai() -> None:
    for path in Path("src/erp_harness.runtime").rglob("*.py"):
        source = path.read_text(encoding="utf-8")
        assert "from erp_harness.providers" not in source, path
        assert "import erp_harness.providers" not in source, path


def test_pi_ai_reexports_canonical_event_classes() -> None:
    from erp_harness.providers.events import TextDeltaEvent as public
