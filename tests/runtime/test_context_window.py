...
from erp_harness.tools.filesystem import create_coding_tools
...
def test_context_token_estimate_includes_system_messages_and_tools(tmp_path: Path) -> None:
    ...
    assert estimate > estimate_text_tokens("You are Pi.hellohi")
...
def test_context_usage_estimate_reports_breakdown(tmp_path: Path) -> None:
    ...
    usage = estimate_context_usage(system="You are Pi.", messages=messages, tools=tools)

    assert isinstance(usage, ContextUsageEstimate)
    assert usage.message_count == 2
    assert usage.tool_count == len(tools)
    assert usage.system_tokens == estimate_text_tokens("You are Pi.")
    ...
    assert estimate_context_tokens(system="You are Pi.", messages=messages, tools=tools) == (
        usage.total_tokens
    )
...
def test_compaction_serialization_matches_pi_transcript_shape() -> None:
    messages = (
        UserMessage(content="Inspect."),
        AssistantMessage(
            content=[
                ThinkingContent(thinking="Plan first."),
                TextContent(text="Reading."),
                ToolCall(id="call", name="read", arguments={"path": "a.py", "line": 2}),
            ]
        ),
        ToolResultMessage(tool_call_id="call", tool_name="read", content="contents"),
    )

    assert serialize_messages_for_compaction(messages) == (
        "[User]: Inspect.\n\n"
        "[Assistant thinking]: Plan first.\n\n"
        "[Assistant]: Reading.\n\n"
        '[Assistant tool calls]: read(path="a.py", line=2)\n\n'
