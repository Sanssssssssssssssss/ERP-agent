...
import erp_harness.providers.config as provider_config
from erp_harness.providers.credentials import FileCredentialStore, OAuthCredential
from erp_harness.context.paths import RuntimePaths
from erp_harness.providers.catalog import ModelCostTier
...
def test_stale_preferences_cannot_restore_removed_codex_alias(tmp_path: Path) -> None:
    ...
    settings = load_provider_settings(RuntimePaths(home=pi_home))
...
def test_load_provider_settings_accepts_huggingface_inference_provider_preferences(
    ...
    provider = load_provider_settings(RuntimePaths(home=pi_home)).get_provider("huggingface")
...
def test_load_provider_settings_missing_file_uses_openai_default(tmp_path: Path) -> None:
    settings = load_provider_settings(RuntimePaths(home=tmp_path / ".pi-agent"))
...
def test_load_provider_settings_accepts_provider_preferences_with_user_catalog(
    ...
    pi_home = tmp_path / ".pi-agent"
    pi_home.mkdir()
    (pi_home / "catalog.toml").write_text(
        """
schema_version = 1
...
def test_provider_settings_ignore_unknown_fields(tmp_path: Path) -> None:
    settings = provider_settings_from_json(
        ...
        paths=RuntimePaths(home=tmp_path / ".pi-agent"),
    )
...
def test_save_and_load_provider_settings_round_trip(
    ...
    assert path == tmp_path / ".pi-agent" / "providers.json"
...
def test_kimi_k3_maps_thinking_levels_to_reasoning_effort(
    ...
    settings = load_provider_settings(RuntimePaths(home=Path("/missing")))
...
def test_huggingface_glm_5_2_does_not_send_unverified_medium_effort(
    ...
    monkeypatch.setenv("HF_TOKEN", "test-key")
    provider = load_provider_settings(RuntimePaths(home=Path("/missing"))).get_provider("huggingface")
...
""".strip(),
    ...
    migrated = json.loads((pi_home / "providers.json").read_text())
    assert migrated["schema_version"] == 2
    assert "providers" not in migrated
    assert (pi_home / "providers.json.bak").exists()
    assert not (pi_home / "catalog.toml").exists()
...
def test_load_provider_settings_migrates_custom_provider_to_catalog(
    ...
    (pi_home / "providers.json").write_text(json.dumps(original), encoding="utf-8")
    ...
    assert provider.headers == {"X-Test": "yes"}
    assert json.loads((pi_home / "providers.json.bak").read_text()) == original
    migrated = json.loads((pi_home / "providers.json").read_text())
    assert migrated["schema_version"] == 2
    assert migrated["provider_preferences"]["local"]["default_model"] == "qwen"
    catalog = (pi_home / "catalog.toml").read_text()
...
def test_legacy_migration_aborts_before_changes_when_backup_fails(
    ...
    settings_path = pi_home / "providers.json"
    ...
    with pytest.raises(PermissionError, match="backup denied"):
        load_provider_settings(RuntimePaths(home=pi_home))

    assert json.loads(settings_path.read_text()) == original
    assert not (pi_home / "providers.json.bak").exists()
    assert not (pi_home / "catalog.toml").exists()
...
""".strip(),
    ...
    store = FileCredentialStore(pi_home / "credentials.json")
