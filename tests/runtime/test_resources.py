...
from erp_harness.context.project import discover_project_context
...
def test_project_trust_preserves_explicit_resource_opt_out(tmp_path: Path, filename: str) -> None:
    ...
    paths = ResourcePaths(root=tmp_path / "home", cwd=cwd, project_resources_enabled=False)
...
def test_project_resource_opt_in_still_requires_trust(tmp_path: Path, trusted: bool) -> None:
    ...
    paths = ResourcePaths(root=tmp_path / "home", cwd=cwd)
...
def test_resource_paths_use_pi_subdirectories(tmp_path: Path) -> None:
    paths = ResourcePaths(root=tmp_path, agents_root=None)
...
def test_resource_paths_include_agents_and_project_directories(tmp_path: Path) -> None:
    ...
    paths = ResourcePaths(
        ...
        cwd=cwd,
        paths=RuntimePaths(home=pi_home, agents_home=agents_home),
    )
...
def test_system_prompt_files_use_project_values_before_user_values(tmp_path: Path) -> None:
    ...
    resources = discover_system_prompt_resources(
        ResourcePaths(root=pi_home, agents_root=agents_home, cwd=cwd)
    )
...
def test_overlapping_user_and_project_append_path_is_loaded_once(tmp_path: Path) -> None:
    ...
    resources = discover_system_prompt_resources(
        ResourcePaths(root=pi_home, agents_root=None, cwd=tmp_path)
    )
...
def test_explicit_replacement_shadows_file_but_append_files_still_load(tmp_path: Path) -> None:
    ...
    resources = discover_system_prompt_resources(
        ResourcePaths(root=pi_home, agents_root=None),
...
def test_selected_system_prompt_file_must_be_readable_utf8(tmp_path: Path) -> None:
    ...
    with pytest.raises(ResourceError, match="Could not read replacement system prompt file"):
        discover_system_prompt_resources(ResourcePaths(root=pi_home, agents_root=None))
