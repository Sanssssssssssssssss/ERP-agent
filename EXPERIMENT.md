...
本文件是**实现路线图，不是完成报告**。用户已授权按阶段执行长任务：每阶段通过离线检查、真实 Odoo 检查、有界模型 A/B 和独立子 agent 复查后，才能进入下一阶段。当前正在验收第一阶段，第二至第七阶段尚未实现；进展和失败记录见 [STAGE1_RESULT.md](STAGE1_RESULT.md)。
...
当前执行阶段。初始预算为 case 2262 一对 A/B，每组最多 1,800 秒；不跑 C、不跑 Compiler。无模型检查失败先修复，不消耗模型预算。
