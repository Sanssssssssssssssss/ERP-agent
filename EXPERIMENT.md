...
本文件是**实现路线图，不是完成报告**。用户已授权按阶段执行长任务：每阶段通过离线检查、真实 Odoo 检查、有界模型 A/B 和独立子 agent 复查后，才能进入下一阶段。第一、第二阶段均已验收；当前进入第三阶段，第四至第七阶段尚未实现。证据和历史失败见 [STAGE1_RESULT.md](STAGE1_RESULT.md) 与 [STAGE2_RESULT.md](STAGE2_RESULT.md)。
