...
本文件是**实现路线图，不是完成报告**。用户已授权按阶段执行长任务：每阶段通过离线检查、真实 Odoo 检查、有界模型 A/B 和独立子 agent 复查后，才能进入下一阶段。第一至第四阶段均已验收；当前进入第五阶段，第六至第七阶段尚未实现。证据和历史失败见 [STAGE1_RESULT.md](STAGE1_RESULT.md)、[STAGE2_RESULT.md](STAGE2_RESULT.md)、[STAGE3_RESULT.md](STAGE3_RESULT.md) 与 [STAGE4_RESULT.md](STAGE4_RESULT.md)。
...
**当前下一步：** 从第五阶段第一个检查点开始，先建立现有 MCP 能力清单并逐项确认原生去向，再迁移诊断与业务封装。SOP 和动态工具集合保持后续独立检查点，不与能力覆盖同时改；第六至第七阶段尚未完成。
