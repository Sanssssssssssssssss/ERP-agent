...
本文件是**实现路线图，不是完成报告**。用户已授权按阶段执行长任务：每阶段通过离线检查、真实 Odoo 检查、有界模型 A/B 和独立子 agent 复查后，才能进入下一阶段。第一阶段已验收：r3 A/B 均自然结束、100 分、62 条适用规则全通过，独立复查无阻断；当前进入第二阶段，第三至第七阶段尚未实现。证据和历史失败见 [STAGE1_RESULT.md](STAGE1_RESULT.md)。
...
**当前下一步：** 保存第一阶段已验收检查点，再实现第二阶段完整读取与统一策略。仍逐阶段验收，不跳过检查去搭后续框架；第三至第七阶段尚未完成。
